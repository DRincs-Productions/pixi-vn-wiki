...
存档与读档是视觉小说的核心功能之一——它让玩家能随时保存进度，并在之后继续体验故事。 This is essential for story-driven games, allowing players to resume the story from where they left off.
...
要加载存档时，使用`Game.restoreGameState()`。 This function has the following parameters:

* `saveData`：要恢复的存档数据对象。
* `navigate`：加载完成后跳转至<DynamicLink href="/start/interface-navigate">指定路径的函数</DynamicLink>。
...
export async function saveGameToIndexDB(
    ...
    if (item.id === undefined) {
        let lastSave = await getLastRowFromIndexDB<
...
> {
    let list = await getListFromIndexDB<GameSaveData & { id: number }>(
        INDEXED_DB_SAVE_TABLE,
        {
...
由于支持开发网页游戏，玩家意外关闭浏览器导致游戏进度丢失的情况较为常见。
