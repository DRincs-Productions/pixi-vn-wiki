...
<Accordion title="创建存档" id="create">

:::tip[模板]

This functionality is already implemented in all templates, via the `createGameSave` function. 具体实现可参考`utils/save-utility.ts`。
...
export async function createGameSave(options?: {
    image?: string;
    name?: string;
}): GameSaveData {
...
IndexedDB is a browser API for storing large amounts of structured data, including files/blobs. 。
...
<Accordion title="继续" id="continue">
