...
- `id`: A unique identifier (string). 用于在游戏中引用角色（必须是唯一的）。 如果你想创建一个[带有“情感”的角色，你可以传递一个对象](#character-emotions)。
- `props`：一个包含角色属性的对象：
  - `name`：角色的名字。
  - `surname`（可选）：角色的姓氏。
  - `age`（可选）：角色的年龄。
...
你还可以创建一个函数来加载角色。 重要的是，在游戏中使用角色之前至少需要调用一次该函数，否则这些角色将会无法使用。
...
此方法可以与_ink_语法一起使用。 查看更多<DynamicLink href="/ink/character#use">这里</DynamicLink>。
...
此方法可以与_ink_语法一起使用。 在<DynamicLink href="/ink/character#edit">这里</DynamicLink>查看更多。
...
这里有一个`CharacterBaseModel`类的简化实现，以便于更好地理解存储在游戏存储中的属性：
...
此方法可以与_ink_语法一起使用。 查看更多<DynamicLink href="/ink/character#character-emotions">这里</DynamicLink>。
