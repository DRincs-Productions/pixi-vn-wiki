---
title: Esempi di schermate UI
description: Fornisce esempi di schermate UI per Pixi’VN, mostrando componenti React e Vue, e illustrando hook (ad es. useQueryDialogue, useQueryChoiceMenuOptions, useQueryCanGoNext) e l'integrazione con la narrazione per scelte, avanti/indietro, richieste di input e cronologia.
---

:::tip[IA]

Puoi usare il pulsante qui sopra per creare la tua schermata UI con l'IA.

:::

:::tip

Questi esempi usano semplici primitive UI (`Button`, `Card`, `Avatar`, `Dialog`, `Input`, ...) importate da una cartella `components/ui`, simile a quanto offerto da [shadcn/ui](https://ui.shadcn.com) (React) o [shadcn-vue](https://www.shadcn-vue.com). Sostituiscile con i componenti equivalenti della tua libreria UI.

:::

<Accordions>

<Accordion title="Schermata UI del dialogo narrativo" id="narrative-dialogue">
...
<Accordion title="Schermata UI del menu di scelta" id="choice-menu">
...
<Accordion title="Continua cliccando sullo schermo" id="go-next">

Invece di un pulsante "avanti" dedicato, racchiudi la schermata narrativa in un overlay che fa avanzare la narrazione ogni volta che il giocatore clicca in un punto qualsiasi di essa.
...
Racchiudi la schermata di dialogo narrativo con esso, in modo che un clic in un punto qualsiasi dello schermo faccia avanzare la narrazione (elementi come il menu di scelta, il pulsante indietro e la finestra di input chiamano `stopPropagation()` in modo che interagire con essi non attivi anche un "avanti"):
...
<Accordion title="Pulsante indietro" id="go-back">
...
<Accordion title="Finestra di dialogo UI per l'input" id="input-prompt">
