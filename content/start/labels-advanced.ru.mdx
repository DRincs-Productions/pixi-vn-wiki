...
You can set this function by passing it to the `onLoadingLabel` property in the `label` options. Эта функция принимает следующие параметры:
...
`onStepStart` is a function executed before each `step` of a `label`. Set it via the `onStepStart` property in the `label` options. Эта функция принимает следующие параметры:
...
`onStepEnd` is a function executed after each `step` of a `label`. Set it via the `onStepEnd` property in the `label` options. Эта функция принимает следующие параметры:
