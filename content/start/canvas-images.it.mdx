...
- `imageUrl` (Optional): The URL or path of the image. Se hai inizializzato la <DynamicLink href="/start/assets-management#initialize-the-asset-matrix-at-project-start">matrice delle risorse</DynamicLink>, puoi utilizzare l'alias della texture. If you don't provide the URL, then the alias is used as the URL.
- `options` (Optional): It corresponds to the `ImageSpriteOptions` interface.
