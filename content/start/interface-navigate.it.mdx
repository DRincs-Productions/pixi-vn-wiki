...
**What is the URL paths/routes?** The **URL paths/routes** are the parts of the URL that come after the domain. For example, in the URL `https://example.com/path/to/page`, the route is `/path/to/page`. In genere, in tutti i progetti web front-end (client), ogni percorso è collegato a una pagina web (schermata dell'interfaccia utente).
...
In all templates, the `StepLabelProps` interface is already extended to include a `navigate` function. La puoi trovare nel file `pixi-vn.d.ts`.
