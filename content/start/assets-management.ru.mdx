...
Во всех шаблонах при запуске `части` будет запущена фоновая загрузка пакета (если он существует) с идентификатором, соответствующим `части`. Поэтому достаточно **создать манифест с пакетом для каждой `части`**. Вот реализация, используемая для этого:
