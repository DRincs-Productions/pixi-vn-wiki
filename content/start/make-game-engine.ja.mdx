...
The first `step` is to create a new project. テンプレートから新しいプロジェクトを作成する方法についての詳細は <DynamicLink href="/start#project-initialization">こちら</DynamicLink> を参照してください。 The "Game Engine" template is a simple project that contains the basic structure of a game engine. By default, it exports the basic functionality of pixi-vn while removing anything unused from the package.
