...
The simplest and fastest method to show an video on the canvas is to use the `showVideo` function. Questa funzione è una combinazione delle funzioni `load` e [`canvas.add`](/start/canvas-functions#add-a-canvas-component).
