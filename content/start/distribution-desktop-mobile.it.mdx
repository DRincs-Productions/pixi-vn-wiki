...
However, if you do not want to create a project with large customizations, I recommend using the <DynamicLink href="/start/getting-started#project-initialization">multi-device templates</DynamicLink>, in which Tauri has been added in order to allow both the development of a Web app and the development of desktop and mobile applications.
...
The <DynamicLink href="/start/getting-started#project-initialization">multi-device templates</DynamicLink> have inside the file `/.github/workflows/tauri.yml` which has the purpose of creating a release not inside the installation packages for all OS.
