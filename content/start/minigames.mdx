...
The beauty of an interactive story compared to a normal text is that you can take a break every now and then to interact with minigames related to the story.

I recommend using the following tips:

* Use the components provided by `PIXI` component, such as `Graphics`, `Sprite`, and `Text`, to create your minigame. `PIXI` is the PixiJS object, so you can use all its features.
* Use the <DynamicLink href="/start/interface#adding-pixijs-ui-layers">PixiJS Layers</DynamicLink> to create a separate layer for your minigame. This allows you to manage the minigame independently of the main visual novel interface.
* Use the `PIXI.Ticker` class to manage the game loop and update the game state.
* Use the `window.addEventListener` method to handle user input, such as keyboard or mouse events.
* Use the HTML elements to create the UI, such as buttons, score display, and game over messages.
* If you are using PixiJS Layers it is very important to consider that Pixi’VN don't save the state of the layers, so you need to manage the state of the minigame yourself. This means that you need to save the state of the minigame in a variable and restore it when the minigame is restarted.

<Callout type="info" name="Template">
...
// PIXI is pixi.js, so you can use all its features
