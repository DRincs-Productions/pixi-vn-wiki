...
You can set this function by passing it to the `onLoadingLabel` property in the `label` options. この関数は以下のパラメータを持ちます：
...
`onStepStart` is a function executed before each `step` of a `label`. Set it via the `onStepStart` property in the `label` options. この関数は以下のパラメータを持ちます：
...
`onStepEnd` is a function executed after each `step` of a `label`. Set it via the `onStepEnd` property in the `label` options. この関数は以下のパラメータを持ちます：
