...
Para ello, estableceremos [`narration.choices`](/jsdoc/pixi-vn/index/interfaces/NarrationChoicesInterface#list) con un array de opciones de elección creadas con [`newChoiceOption`](/jsdoc/pixi-vn/index/functions/newChoiceOption) (para saltar o llamar a un `label`) y [`newCloseChoiceOption`](/jsdoc/pixi-vn/index/functions/newCloseChoiceOption) (para cerrar el menú de elección sin navegar a ningún sitio).
...
En este ejemplo, usamos la transición move in/out para hacer que `james` entre en la escena desde fuera del canvas. In *ink*, this is done by appending `with movein` (and its parameters) to the `# show` command, while in JS/TS you can use the `transitions.moveIn`/`transitions.moveOut` functions.
...
To take `steph` out/in, I will use the `transitions.moveOut` and `transitions.moveIn` functions. Para el efecto espejo, usaré la función `canvas.animate`.
...
Uso la opción `completeOnContinue: true` para asegurarme de que la animación se complete antes de que se ejecute el siguiente `step`. For `transitions.moveIn`, being a transition, `completeOnContinue` is true by default.
