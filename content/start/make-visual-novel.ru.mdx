...
Первым шагом является создание нового проекта. You can do this by running:
...
Теперь мы определим персонажей этой истории. To do this, we will define in the `/content/characters.ts` file the characters that we will be using.
...
Более подробную информацию о создании и использовании персонажей можно найти здесь: <DynamicLink href="/start/character">Characters</DynamicLink>
...
<Accordion title="Умная загрузка ассетов" id="smart-asset-loading">

In our case we saved the game images on a hosting service. По этой причине загрузка ассетов не является мгновенной.
...
Теперь пришло время подумать и о визуальной части. We will add the background and character images to the visual novel canvas.
...
Чтобы сделать визуальную новеллу более динамичной, можно использовать анимации. I recommend using TypeScript if you need to set many properties, as this gives you more control, more functionality, and type feedback.
...
Подробнее о работе с анимациями см. <DynamicLink href="/start/canvas-animations-effects">здесь</DynamicLink>.
