...
One of the first `steps` is choosing where to save your visual novel assets. В данном случае мы сохраним текстуры в хранилище Firebase (хостинговый сервис). You can use any hosting service you prefer or even save the assets locally in the project, read more about it <DynamicLink href="/start/assets">here</DynamicLink>.
...
В нашем случае спрайты персонажей состоят из 3 изображений: тела, глаз и рта. Then we use <DynamicLink href="/start/canvas-image-container">ImageContainer</DynamicLink> to compose the character. You can find more information on how to add canvas components in <DynamicLink href="/start/canvas-components">this documentation</DynamicLink>.
...
Чтобы сделать визуальную новеллу более динамичной, можно использовать переходы для показа изображений. You can find more information about using transitions <DynamicLink href="/start/canvas-transition">here</DynamicLink>.
...
Чтобы сделать визуальную новеллу более динамичной, можно использовать анимацию. You can find more information about how to use animations <DynamicLink href="/start/canvas-animations-effects">here</DynamicLink>.
...
For taking `steph` out/in I will use the `moveOut` and `moveIn` functions. For the mirror effect I will use the `canvas.animate` function.
