...
Теперь вы можете вызывать этот `label` (`animation_01`) из основного `label` (`start`). (As explained <DynamicLink href="/ink/ink-label#use-the-call-script">here</DynamicLink>, from _ink_ you can call `labels` written in ts and vice versa.)
