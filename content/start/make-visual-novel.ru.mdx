...
The first `step` is to create a new project. You can find more information on how to create a new project starting from a template <DynamicLink href="/start/getting-started#project-initialization">here</DynamicLink>. Мы будем использовать шаблон "Визуальная новелла - React".
...
В визуальных новеллах часто бывает полезно вставить текст в текущий диалог. For example, to pause a conversation and have it continue in a subsequent `step`. To do this, we can use the <DynamicLink href="/start/dialogue#dialogue-glue">glue functionality</DynamicLink>.
...
One of the first `steps` is choosing where to save your visual novel assets. В данном случае мы сохраним текстуры в хранилище Firebase (хостинговый сервис).
...
Чтобы игрок не получал слишком много загрузок, мы должны сгруппировать их в определенные фазы игры. In my case I will load the most used images at the start of the `label`.
...
In my case my animation will take `steph` out of the scene and reinsert her in the next `step`. Я также отзеркалю её по оси x, чтобы убедиться, что она направлена в нужную сторону.

For taking `steph` out/in I will use the `moveOut` and `moveIn` functions. Для зеркального эффекта я буду использовать тикер `ZoomTicker`.
...
Поэтому в моем случае я использую перед функцией `moveIn` функцию `addTicker`, чтобы добавить тикер `ZoomTicker`. This way `steph` will be mirrored on the x-axis after the transition is complete.

Also since I will use Typescript for this animation, I created a `label` for this animation. Чтобы его можно было вызывать и из других языков, не являющихся JS/TS.
...
Now I can call this `label` `animation_01` from the main `label` `start`. (As explained <DynamicLink href="/ink/ink-label#use-the-call-script">here</DynamicLink> from _ink_ you can call `labels` written in ts and vice versa.)
