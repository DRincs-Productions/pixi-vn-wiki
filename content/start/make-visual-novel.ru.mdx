...
It is not advisable to create very long `labels` (even for linear visual novels), but it is advisable to create multiple small `labels` and "call" them when needed with the <DynamicLink href="/start/labels-flow">game flow control features</DynamicLink> (<DynamicLink href="/ink/ink-label">_ink_ knot (or `label`)</DynamicLink>).
