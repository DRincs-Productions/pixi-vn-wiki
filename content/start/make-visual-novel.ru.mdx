...
В этом примере мы используем переход "движение внутрь/наружу" (move in/out), чтобы заставить `james` войти в сцену извне холста. In *ink*, this is done by appending `with movein` (and its parameters) to the `# show` command, while in JS/TS you can use the `transitions.moveIn`/`transitions.moveOut` functions.
...
To take `steph` out/in, I will use the `transitions.moveOut` and `transitions.moveIn` functions. Для эффекта зеркального отражения я буду использовать функцию `canvas.animate`.
...
Я использую опцию `completeOnContinue: true`, чтобы убедиться, что анимация завершится до выполнения следующего нарративного шага (`step`). For `transitions.moveIn`, being a transition, `completeOnContinue` is true by default.
