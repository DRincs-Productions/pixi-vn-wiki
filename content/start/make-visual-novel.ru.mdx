...
Forwarding I used the `forceCompleteBeforeNext: true` option to ensure that the animation completes before the next `step` is executed. While for `move In`, being a transaction, `force Complete Before Next` is true by default.
