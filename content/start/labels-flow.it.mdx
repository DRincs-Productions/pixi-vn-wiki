...
- `props`: the properties that will be passed to the label. Its interface corresponds to <DynamicLink href="/start/labels#override-steplabelprops">`StepLabelProps`</DynamicLink>.
...
`narration.callLabel` returns a <DynamicLink href="/start/labels#all-steps-result">result of first step of the called label</DynamicLink>.
...
Remember that if you execute the `narration.callLabel` inside a step, you should return the <DynamicLink href="/start/labels#all-steps-result">result of first step of the called label</DynamicLink> and use `await`. The reason is that it might generate a wrong history.
...
`narration.jumpLabel` returns a <DynamicLink href="/start/labels#all-steps-result">result of first step of the called label</DynamicLink>.
...
Remember that if you execute the `narration.jumpLabel` inside a step, you should return the <DynamicLink href="/start/labels#all-steps-result">result of first step of the called label</DynamicLink> and use `await`. The reason is that it might not await the first step of the new label.
...
In parameters you must pass a function `navigate: (path: string) => void` that will be called with the <DynamicLink href="/start/interface#what-is-the-url-path-and-routes">URL Path or Route</DynamicLink> of the previous step, so you can to <DynamicLink href="/start/interface-navigate">navigate to the previous UI screen</DynamicLink>.
...
**If you call this function and after that you don't call any label, the <DynamicLink href="/start/other-narrative-features#how-manage-the-end-of-the-game">game will end</DynamicLink>.**
