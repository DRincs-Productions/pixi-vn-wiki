...
È possibile utilizzare la proprietà `narration.canContinue` per verificare se lo `step` successivo può essere eseguito.

`narration.canContinue` è `false` quando:
...
Se si eseguono più richieste `stepHistory.back()` e `narration.continue()` in modo sincrono, il sistema le metterà in coda internamente e calcolerà il delta per eseguire solo i processi necessari.

È possibile implementare le funzionalità di rollback e rollforward utilizzando le funzioni `stepHistory.back()` e `narration.continue()`.
