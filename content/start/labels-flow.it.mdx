...
If you execute `narration.callLabel` inside a `step`, you should return the result of `narration.callLabel` and use `await`. In questo modo si garantisce una corretta gestione della cronologia.
...
If you execute `narration.jumpLabel` inside a `step`, you should return the result of `narration.jumpLabel` and use `await`. In questo modo si garantisce che si attenda il primo `step` della nuova `label`.
...
You can find the example of the go next button in the <DynamicLink href="/start/interface-examples#go-next">interface examples</DynamicLink> section.
...
1. `await narration.callLabel(label2, props)` chiama label2 e attende il suo primo `step`. (1 item in the queue)
