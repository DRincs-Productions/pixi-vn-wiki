---
title: UI-Bildschirmbeispiele
description: Bietet UI-Bildschirmbeispiele für Pixi’VN, zeigt React- und Vue-Komponenten und veranschaulicht Hooks (z. B. useQueryDialogue, useQueryChoiceMenuOptions, useQueryCanGoNext) sowie die Integration mit der Erzählung für Auswahlmöglichkeiten, Weiter/Zurück, Eingabeaufforderungen und Verlauf.
---

:::tip[KI]

Du kannst die Schaltfläche oben verwenden, um deinen eigenen KI-UI-Bildschirm zu erstellen.

:::

:::tip

Diese Beispiele verwenden einfache UI-Primitive (`Button`, `Card`, `Avatar`, `Dialog`, `Input`, ...) die aus einem `components/ui`-Ordner importiert werden, ähnlich dem, was [shadcn/ui](https://ui.shadcn.com) (React) oder [shadcn-vue](https://www.shadcn-vue.com) bieten. Ersetze sie durch die entsprechenden Komponenten deiner eigenen UI-Bibliothek.

:::

<Accordions>

<Accordion title="UI-Bildschirm für Erzähldialog" id="narrative-dialogue">
...
<Accordion title="UI-Bildschirm für Auswahlmenü" id="choice-menu">
...
<Accordion title="Durch Klicken auf den Bildschirm fortfahren" id="go-next">

Statt einer dedizierten „Weiter"-Schaltfläche umschließt du den Erzählbildschirm mit einem Overlay, das die Erzählung fortsetzt, sobald der Spieler irgendwo darauf klickt.
...
Umschließe den Erzähldialog-Bildschirm damit, sodass ein Klick irgendwo auf dem Bildschirm die Erzählung fortsetzt (Elemente wie das Auswahlmenü, der Zurück-Button und der Eingabedialog rufen `stopPropagation()` auf, damit die Interaktion mit ihnen nicht ebenfalls ein „Weiter" auslöst):
...
<Accordion title="Zurück-Button" id="go-back">
...
<Accordion title="UI-Dialog für Eingabeaufforderung" id="input-prompt">
