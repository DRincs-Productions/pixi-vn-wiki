...
* `props`: An object with the character's properties. The properties are:
...
`CharacterBaseModel` is a <DynamicLink href="/start/stored-classes">stored class</DynamicLink>, which means that its properties are saved in <DynamicLink href="/start/storage">game storage</DynamicLink>.
