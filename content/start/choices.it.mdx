...
- `options` (Opzionale): Un oggetto con le opzioni di `choice`:
  - `type`: How the <DynamicLink href="/start/labels-flow#run-a-label">label is called</DynamicLink>. Può essere `call` o `jump`. Il valore predefinito è `call`.
...
Per ottenere il menu di scelta corrente, usa `narration.choiceMenuOptions`. This returns an array of `newChoiceOption` and/or `newCloseChoiceOption`.
