...
To require the player to make a choice, set `narration.choices` to an array of `StoredChoiceInterface`. Per creare un oggetto `StoredChoiceInterface`, usa:
...
- `props`: Le proprietà passate alla `label`. If the `label` does not require parameters, pass an empty object `{}`.
...
To get the current choice menu, use `narration.choices`. Restituisce un array di `StoredChoiceInterface`.
