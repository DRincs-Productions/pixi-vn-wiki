---
title: UI画面の例
description: Pixi’VN向けのUI画面の例を提供し、ReactおよびVueコンポーネントを示すとともに、フック（useQueryDialogue、useQueryChoiceMenuOptions、useQueryCanGoNextなど）や、選択肢、次へ/戻る、入力プロンプト、履歴に関するナレーションとの統合を実演します。
---

:::tip[AI]

上のボタンを使って、独自のAI UI画面を作成できます。

:::

:::tip

これらの例では、シンプルなUIプリミティブ（`Button`、`Card`、`Avatar`、`Dialog`、`Input`など）を使用しています `components/ui`フォルダからインポートされており、[shadcn/ui](https://ui.shadcn.com)（React）や[shadcn-vue](https://www.shadcn-vue.com)が提供するものと似ています。 自分のUIライブラリの同等のコンポーネントに置き換えてください。

:::

<Accordions>

<Accordion title="ナラティブダイアログUI画面" id="narrative-dialogue">
...
<Accordion title="選択肢メニューUI画面" id="choice-menu">
...
<Accordion title="画面をクリックして続行" id="go-next">

専用の「次へ」ボタンの代わりに、プレイヤーが画面のどこかをクリックするたびにナレーションを進めるオーバーレイでナラティブ画面をラップします。
...
ナラティブダイアログ画面をこれでラップすることで、画面のどこをクリックしてもナレーションが進みます（選択肢メニュー、戻るボタン、入力ダイアログなどの要素は`stopPropagation()`を呼び出すため、それらを操作しても「次へ」がトリガーされません）：
...
<Accordion title="戻るボタン" id="go-back">
...
<Accordion title="入力プロンプトダイアログUI" id="input-prompt">
