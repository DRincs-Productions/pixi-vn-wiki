---
title: Exemples d'écrans UI
description: Fournit des exemples d'écrans UI pour Pixi’VN, montrant des composants React et Vue, et illustrant des hooks (par ex. useQueryDialogue, useQueryChoiceMenuOptions, useQueryCanGoNext) ainsi que l'intégration avec la narration pour les choix, suivant/précédent, les invites de saisie et l'historique.
---

:::tip[IA]

Vous pouvez utiliser le bouton ci-dessus pour créer votre propre écran UI avec l'IA.

:::

:::tip

Ces exemples utilisent des primitives UI simples (`Button`, `Card`, `Avatar`, `Dialog`, `Input`, ...) importées depuis un dossier `components/ui`, similaire à ce que proposent [shadcn/ui](https://ui.shadcn.com) (React) ou [shadcn-vue](https://www.shadcn-vue.com). Remplacez-les par les composants équivalents de votre propre bibliothèque UI.

:::

<Accordions>

<Accordion title="Écran UI de dialogue narratif" id="narrative-dialogue">
...
<Accordion title="Écran UI du menu de choix" id="choice-menu">
...
<Accordion title="Continuer en cliquant sur l'écran" id="go-next">

Au lieu d'un bouton « suivant » dédié, enveloppez l'écran narratif dans un overlay qui fait avancer la narration à chaque clic du joueur n'importe où dessus.
...
Enveloppez l'écran de dialogue narratif avec celui-ci, afin qu'un clic n'importe où sur l'écran fasse avancer la narration (des éléments comme le menu de choix, le bouton retour et la boîte de dialogue de saisie appellent `stopPropagation()` afin que l'interaction avec eux ne déclenche pas aussi un « suivant »):
...
<Accordion title="Bouton retour" id="go-back">
...
<Accordion title="Boîte de dialogue UI de saisie" id="input-prompt">
