...
Dans cet exemple, nous utilisons la transition move in/out pour faire entrer `james` dans la scène depuis l'extérieur du canvas. In *ink*, this is done by appending `with movein` (and its parameters) to the `# show` command, while in JS/TS you can use the `transitions.moveIn`/`transitions.moveOut` functions.
...
To take `steph` out/in, I will use the `transitions.moveOut` and `transitions.moveIn` functions. Pour l'effet miroir, j'utiliserai la fonction `canvas.animate`.
...
Then, I use the `transitions.moveIn` function to move `steph` into the scene, passing the `tickerId` of the animation so it can be resumed after the transition is complete.

J'utilise l'option `completeOnContinue: true` pour m'assurer que l'animation se termine avant que le `step` suivant ne soit exécuté. For `transitions.moveIn`, being a transition, `completeOnContinue` is true by default.
...
import { canvas, ImageContainer, newLabel, transitions } from "@drincs/pixi-vn";
...
const startLabel = newLabel("start", [
    ...
    async () => {
        ...
        transitions.moveOut("steph", { direction: "left", speed: 300 });
...
const startLabel = newLabel("start", [
    async () => {
        ...
        await transitions.moveIn("james", {
            value: ["m01-body", "m01-eyes-smile", "m01-mouth-neutral01"],
            options: { xAlign: 0.5, yAlign: 1 },
        });
        narration.dialogue = {
    ...
    async () => {
        ...
        transitions.moveOut("sly", {
            direction: "right",
            ease: "anticipate",
            duration: 0.5,
        });
        transitions.moveOut("steph", {
            direction: "left",
            ease: "easeInOut",
            duration: 0.5,
            delay: 0.1,
...
const secondPart = newLabel("second_part", [
    async ({ t }) => {
        ...
        await transitions.showWithFade("bg", text);
        text.align = 0.5;
    },
    async () => {
        sound.resumeAll();
        await transitions.showWithFade("bg", "bg02-dorm");
