...
To create a save, use `Game.exportGameState()`. It returns an object with the current game state, which you can then store in a file, a database, or any storage of your choice.
...
To generate a screenshot, use `canvas.extractImage()`. This returns a base64 string of the current canvas.
...
To restore a save, use `Game.restoreGameState()`. It takes the following parameters:

- `saveData`: The save data object to restore the game state from.
- `navigate`: <DynamicLink href="/start/interface-navigate">A function to navigate to a specific route</DynamicLink>, called after the save has been restored.
