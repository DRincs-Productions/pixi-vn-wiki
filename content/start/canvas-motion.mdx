...
There are another way to create sequences using the `canvas.animate` function, which is to use a timeline. This is useful when you want to create a sequence of animations that are not necessarily linear. Using an array of keyframes, where each keyframe is an object that contains the properties to animate and their values at different points in time.
