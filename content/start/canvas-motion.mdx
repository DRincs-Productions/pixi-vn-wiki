...
For example, to move a component to position `{ x: 100, y: 50 }` over one second, you pass `{ x: 100, y: 50 }` as the `keyframes` and `{ duration: 1 }` as the `options`. The `keyframes` parameter defines _what_ to reach; `options` defines _how_ to get there.
