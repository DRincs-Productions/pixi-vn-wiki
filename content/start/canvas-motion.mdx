...
* `options`: Additional options for the animation, including duration, easing, and ticker. The following options have been added to `motion`:
  ...
  * `mustBeCompletedBeforeNextStep` (Optional): is a boolean that indicates if the animation must be completed before the next step of the game. If set to `true`, the game will force the animation to complete before moving to the next step.
