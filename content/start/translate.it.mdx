...
In all templates, the `StepLabelProps` interface is already extended to include a `t` function. La puoi trovare nel file `pixi-vn.d.ts`.
...
If you are using ink, you can automatically introduce the strings used in the text encoded with ink into the json file to be translated. You can read more about this <DynamicLink href="/ink/ink-translate">here</DynamicLink>.

</Callout>

The translation of the json file to be translated must be created manually. It is recommended to split the translations into two parts (see `strings_es.json` file):
