...
In our example we would have two variables, `text1` and `text2`, saved in the <DynamicLink href="/start/storage.md">game storage</DynamicLink>, this variables will be updated either by an input in the UI or by a <DynamicLink href="/start/labels">label step</DynamicLink>.

Taking into account that a storage variables can only be changed during a <DynamicLink href="/start/labels.md#next-step-and-go-back">next step, go back</DynamicLink>, <DynamicLink href="/start/labels.md#run-a-label">run label</DynamicLink> or <DynamicLink href="/start/save.md#load">loading a save</DynamicLink> (outside the interface), we will create a `useQueryText1` and `useQueryText2` that will be updated after each <DynamicLink href="/start/labels.md#next-step-and-go-back">next step, go back</DynamicLink>, <DynamicLink href="/start/labels.md#run-a-label">run label</DynamicLink> or <DynamicLink href="/start/save#load">loading a save</DynamicLink>.
