...
Il componente `ImageContainer` estende il componente <DynamicLink href="/start/canvas-components#base-components">`Container`</DynamicLink>, così puoi utilizzare tutti i metodi e le proprietà di `Container`. It allows you to group multiple images into a single component and manipulate them as one.
...
- `options` (Optional): The options for the container, matching the `ImageContainerOptions<ImageSprite>` interface.
