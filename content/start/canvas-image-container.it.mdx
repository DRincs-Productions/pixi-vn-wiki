...
To add a group of images to the canvas, use the `addImageCointainer` function. Questa funzione aggiunge solo il componente al canvas; **non** lo mostra né ne carica la texture. It uses <DynamicLink href="/start/canvas-functions#add-a-canvas-component">`canvas.add`</DynamicLink> to add the component to the canvas.
