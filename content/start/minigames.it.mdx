...
- Utilizza i componenti forniti dall'oggetto `PIXI`, come `Graphics`, `Sprite` e `Text`, per creare il tuo minigioco. `PIXI` è l'oggetto PixiJS, quindi puoi utilizza tutte le sue funzionalità.
...
- Utilizzare la classe `PIXI.Ticker` per gestire il ciclo del gioco e aggiornarne lo stato.
