...
The term `label` is borrowed from Ren'Py, where it acts as a "bookmark" or "landmark" in the story. Nel linguaggio ink, il termine equivalente è `knot`.
...
È possibile passare un tipo alla funzione `newLabel` per aggiungere parametri personalizzati per tale label, oltre a [`StepLabelProps`](#overriding-steplabelprops).
