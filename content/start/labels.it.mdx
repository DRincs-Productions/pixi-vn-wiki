...
- After that, by connecting the <DynamicLink href="/start/labels-flow#continue">Next Step</DynamicLink> function to an event (such as a button click), each time the function is called, the next `step` of the `label` is executed. Alcuni `steps` possono anche avviare altre `labels`.
