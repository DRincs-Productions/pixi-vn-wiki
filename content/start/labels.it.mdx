...
Le visual novel sono solitamente costituite da una successione di screenshot con immagini e testo.

<Callout title="Nomenclature" type="info">
Il termine `label` è stato preso da Ren'Py. This name derives from its function, which is to act as true "bookmarks" or "landmarks." Nel linguaggio ink, il termine `label` corrisponde a `knot`.
