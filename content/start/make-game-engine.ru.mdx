...
Первый шаг - создание нового проекта. You can find more information on how to create a new project starting from a template <DynamicLink href="/start/getting-started#project-initialization">here</DynamicLink>. The "Game Engine" template is a simple project that contains the basic structure of a game engine. By default, it exports the basic functionality of pixi-vn while removing anything unused from the package.
...
Since it has been handled in a generic way, you can implement your own <DynamicLink href="/start/labels#label">`Label`</DynamicLink> (the key element of narration) by extending `LabelAbstract`.
