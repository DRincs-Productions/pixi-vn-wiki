...
<Accordion title="Crea" id="create">

:::tip[Modelli]

This functionality is already implemented in all templates, via the `createGameSave` function. Guarda `utils/save-utility.ts`.
...
<Accordion title="Carica" id="load">
...
IndexedDB is a browser API for storing large amounts of structured data, including files/blobs. Permette di salvare e caricare gli stati del gioco in modo efficiente.
...
<Accordion title="Continuare" id="continue">
