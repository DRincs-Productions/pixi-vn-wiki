...
icon: /react.svg
title: React UI
description: 如何将 Pixi’VN 集成到 React 中，包括设置、模板和最佳 UI 库。 Create a React visual novel or 2D game.
---

**什么是 React？** React 是一个用于构建网站、应用程序等用户界面的 JavaScript 库。 它是免费的、开源的，由 Meta（前身为 Facebook）和一个庞大的社区维护。

您可以在 [React 官网](https://react.dev/) 上了解更多有关 React 的信息。

## 如何将 Pixi’VN 添加到 React 应用程序中

<Callout title="React template" type="info">

有可用的 Pixi’VN 模板，它们使用 React。 您可以在 <DynamicLink href="/start#project-initialization">模板</DynamicLink> 部分中找到有关它们的更多信息。

如果您想将 Pixi’VN 添加到现有的 React 应用程序，请按照以下步骤进行操作。

</Callout>

首先，您需要一个 React 应用程序，并且要 <DynamicLink href="/start#installation">安装 pixi-vn</DynamicLink>。 建议使用 [Vite](https://vite.dev/guide/#scaffolding-your-first-vite-project) 来创建一个新的 React 应用程序。
...
要创建一个美丽的用户界面，您可以使用一些最佳的 React UI 组件库：
