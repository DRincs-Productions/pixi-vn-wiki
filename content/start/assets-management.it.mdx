...
In tutti i template, quando si avvia una `label`, verrà avviato il caricamento in background del bundle (se esiste) con l'id corrispondente alla `label`. So, it is enough to **create a manifest with a bundle for each `label`**. Here is the implementation used to allow this:
