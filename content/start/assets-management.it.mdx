...
To collect the loadings from one step to another of a label in one, it is possible to load all or part of the resources used by a label before it starts. Poiché ciò non ridurrà i tempi di attesa per il giocatore, ma li sommerà a un singolo caricamento, si consiglia di [caricarli in background](#load-assets-in-the-background-at-label-start).
