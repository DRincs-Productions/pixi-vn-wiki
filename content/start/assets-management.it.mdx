...
In all templates, when a `label` is started, the background loading of the bundle (if it exists) with the id corresponding to the `label` will be started. Quindi, è sufficiente **creare un manifest con un bundle per ogni `label`**. Ecco l'implementazione utilizzata per consentirlo:
