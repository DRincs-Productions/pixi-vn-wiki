...
The first `step` is to create a new project. Vous pouvez trouver plus d’informations sur la création d’un projet à partir d’un modèle <DynamicLink href="/start#project-initialization">ici</DynamicLink>. The "Game Engine" template is a simple project that contains the basic structure of a game engine. By default, it exports the basic functionality of pixi-vn while removing anything unused from the package.
