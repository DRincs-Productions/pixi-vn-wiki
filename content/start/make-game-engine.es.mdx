...
The first `step` is to create a new project. Puedes encontrar más información sobre cómo crear un proyecto a partir de una plantilla <DynamicLink href="/start#project-initialization">aquí</DynamicLink>. The "Game Engine" template is a simple project that contains the basic structure of a game engine. By default, it exports the basic functionality of pixi-vn while removing anything unused from the package.
