...
To get a component from the game canvas, you can use `canvas.find`. Se il componente non esiste, restituirà `undefined`.
This function has the following parameters:
...
- `eventClass`: The class that extends `CanvasEvent` and will be executed when the event is triggered. Questa classe deve:
  - avere il metodo `fn` che verrà eseguito quando l'evento viene attivato.
  - avere il decorator `@eventDecorator`. `@eventDecorator` is a decorator that saves the event in memory. It has an optional parameter that is the id used by Pixi’VN to refer to this class (must be unique). Se non si passa l'id, verrà utilizzato come id il nome della classe. (<DynamicLink href="/other-topics/faq#how-enable-the-decorators-in-typescript">How to enable decorators in TypeScript?</DynamicLink>)
