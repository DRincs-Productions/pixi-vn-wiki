...
ビジュアルノベルをより動的にするために、画像表示時にトランジションを使用できます。 トランジションの使用方法についての詳細は <DynamicLink href="/start/canvas-transition">こちら</DynamicLink> を参照してください。
...
ビジュアルノベルをより動的にするために、アニメーションを使用できます。 アニメーションの使用方法についての詳細は <DynamicLink href="/start/canvas-animations-effects">こちら</DynamicLink> を参照してください。
...
`steph` の出入りには `moveOut` と `moveIn` 関数を使用します。 ミラー効果には `canvas.animate` 関数を使用します。
