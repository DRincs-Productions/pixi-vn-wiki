---
title: 最初のビジュアルノベルを作成する
description: Pixi’VN を使用してビジュアルノベルを作成するためのステップバイステップガイド（プロジェクト設定、ナラティブ、アセット、インタラクティビティを含む）。
---
...
**ビジュアルノベルとは？** ビジュアルノベル（VN）はデジタルのインタラクティブフィクションの形式です。 ビジュアルノベルはしばしばビデオゲームの一種と見なされますが、必ずしもゲームとして分類されるわけではありません。 テキストのナラティブに、静止画またはアニメーション画像、そしてさまざまな程度のインタラクティビティを組み合わせた形式です。 英語では “novel game” と呼ばれることもあり、これは和製英語「ノベルゲーム」（noberu gēmu）を転写した表現です。

学習用として、このガイドでは Pixi’VN を使用して 
[Breakdown](https://joshpowlison.itch.io/breakdown) を再現します。 Breakdown は、ビジュアルノベルが備えるべきすべての機能を持つ短い物語です。 Breakdown の作者である Josh Powlison から、教育目的でこのナレーションを使用する許可を得ています ❤️。

Pixi’VN では、1 つまたは複数の <DynamicLink href="/start/narration">利用可能なナラティブ言語</DynamicLink> を選んで物語を書くことができます。このガイドでは、開発の各ステップで利用可能な各言語の例を示します。

## 新しいプロジェクトを作成する

最初のステップは、新しいプロジェクトを作成することです。 テンプレートから新規プロジェクトを作成する方法については、<DynamicLink href="/start#project-initialization">こちらを</DynamicLink>ご覧ください。 このガイドではテンプレート **"Visual Novel - React"** を使用します。

`Visual Novel -> React`

作成が完了したら、プロジェクトのルートにある `README.md` を必ず読んでください。 このファイルには、プロジェクトの使い方に関する重要な情報が記載されています。

本ガイドの場合、プロジェクトを開始するには次のコマンドを実行するだけで十分です：
...
次に、この物語で使用するキャラクターを定義します。 使用するキャラクターは `/values/characters.ts` ファイルで定義します。 キャラクターの作成と使用方法についての詳細は <DynamicLink href="/start/character">Characters</DynamicLink> を参照してください

`mc` とは何でしょうか？ `mc` は "Main Character"（主人公）の一般的な省略形です。 ビジュアルノベルでは、主人公の名前として `mc` を使用するのが一般的です。
...
すべてのテンプレートは <DynamicLink href="/start/markup-markdown">Markdown</DynamicLink> と<DynamicLink href="/start/markup-tailwindcss">Tailwind CSS</DynamicLink>をサポートしているため、これらを使用してナラティブを書けます。

</Callout>

ここから、ビジュアルノベルの <DynamicLink href="/start/narration">ナレーション</DynamicLink> の「初稿」を書き始めます。
最初の <DynamicLink href="/start/labels">`label`</DynamicLink> を `start` として作成し、ゲームの始まりとします。 その後、<DynamicLink href="/start/dialogue">ダイアログ</DynamicLink> を記述して物語を進めます。
...
非常に長い `labels` を作成することは推奨されません（それが線形のビジュアルノベルであっても同様です）。その代わりに、小さな `labels` を複数作成し、必要に応じて <DynamicLink href="/start/labels-flow">ナレーションのフロー制御機能</DynamicLink> を使用して呼び出す方法が一般的です。

このため、このガイドの物語は線形である場合でも 2 つの `labels` に分割します。最初の `label` は先ほど作成したもの（`start`）で、2 つ目は`second_part` と呼ばれます。
...
次に、プレイヤーに物語を続けるかどうかを尋ねます。

これを行うには <DynamicLink href="/start/choices">選択肢メニュー</DynamicLink> を使用します。
...
これには <DynamicLink href="/start/input">Pixi’VN の入力ボックス</DynamicLink> を使用します。

プレイヤーの入力値を取得した後、<DynamicLink href="/start/character#edit">キャラクター名を編集</DynamicLink>できます。
...
その後、更新された名前を <DynamicLink href="/start/character#use">ダイアログ内で使用</DynamicLink> できます。
