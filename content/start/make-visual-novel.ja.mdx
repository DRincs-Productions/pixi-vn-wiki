...
これを行うために、[`narration.choices`](/jsdoc/pixi-vn/index/interfaces/NarrationManagerInterface#choices) に、[`newChoiceOption`](/jsdoc/pixi-vn/index/functions/newChoiceOption)（ナラティブノード（`label`）へジャンプまたは呼び出すため）と [`newCloseChoiceOption`](/jsdoc/pixi-vn/index/functions/newCloseChoiceOption)（どこにも遷移せずに選択肢メニューを閉じるため）で作成した選択肢の配列を設定します。
