...
それでは、ビジュアルノベルの<DynamicLink href="/start/narration">ナラティブ</DynamicLink>の「最初の草案」を書き始めましょう。まず、ゲームの始まりとなる `start` という名前の最初の<DynamicLink href="/start/labels">ナラティブノード（`label`）</DynamicLink>を作成します。その後、ビジュアルノベルで続く<DynamicLink href="/start/dialogue">ダイアログ</DynamicLink>を書いていきます。
...
これを行うために、[`narration.choices`](/jsdoc/pixi-vn/index/interfaces/NarrationChoicesInterface#list) に、[`newChoiceOption`](/jsdoc/pixi-vn/index/functions/newChoiceOption)（ナラティブノード（`label`）へジャンプまたは呼び出すため）と [`newCloseChoiceOption`](/jsdoc/pixi-vn/index/functions/newCloseChoiceOption)（どこにも遷移せずに選択肢メニューを閉じるため）で作成した選択肢の配列を設定します。
...
ここでは、プレイヤーに `mc` の名前を変更できるようにします。そのために、プレイヤーに<DynamicLink href="/start/input">Pixi’VNの機能を使って入力ボックスに入力してもらいます</DynamicLink>。
...
この例では、`james` をキャンバスの外からシーンに登場させるために、ムーブイン/アウトのトランジションを使用します。 In *ink*, this is done by appending `with movein` (and its parameters) to the `# show` command, while in JS/TS you can use the `transitions.moveIn`/`transitions.moveOut` functions.
...
To take `steph` out/in, I will use the `transitions.moveOut` and `transitions.moveIn` functions. ミラー効果には `canvas.animate` 関数を使用します。
...
次のナラティブステップ（`step`）が実行される前にアニメーションが完了することを保証するために、`completeOnContinue: true` オプションを使用します。 For `transitions.moveIn`, being a transition, `completeOnContinue` is true by default.
