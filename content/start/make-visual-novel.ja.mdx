...
description: Pixi'VNを使ってビジュアルノベルを作成するためのステップバイステップガイドです。
---

import { TSVisualNovelExample } from "@/components/vn-examples";
import { InkVisualNovelExample } from "@/components/vn-ink-examples";

このチュートリアルでは、初めてのビジュアルノベルを作成する手順を説明します。

**ビジュアルノベルとは？** ビジュアルノベル（VN）は、デジタルインタラクティブフィクションの一種です。 多くの場合ビデオゲームとして扱われますが、必ずしもゲームとして分類されるわけではありません。 文章による物語と静止画またはアニメーション、そしてさまざまなインタラクションを組み合わせた形式です。 The format is more rarely referred to as novel game, a retranscription of the wasei-eigo term noberu gēmu (ノベルゲーム), which is more often used in Japanese.
