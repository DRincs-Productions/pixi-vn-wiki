...
最初の<DynamicLink href="/start/labels">`label`</DynamicLink>を `start` として作成します。これがゲームの始まりになります。 その後、私たちは視覚小説に続く<DynamicLink href="/start/dialogue">対話</DynamicLink>を書くことができます。
...
非常に長い`ラベル`（線形の視覚小説でも）を作成することは推奨されませんが、複数の小さな`ラベル`を作成し、必要に応じて<DynamicLink href="/start/labels-flow">ナレーションフロー制御機能</DynamicLink>を使用して「呼び出す」ことが推奨されます。

このため、私たちの場合、物語は線形であっても、2つの`labels`に分かれます。最初のものは私たちが作成したばかりのもので、2番目のものは`second_part`と呼ばれます。
...
これから、プレイヤーに視覚小説の第2部を続けるかどうかを尋ねます。

これを行うために、<DynamicLink href="/start/choices">選択メニュー</DynamicLink>を使用します。
...
## キャラクター情報を編集し、それを変数として使用します。

これからプレイヤーに`mc`の名前を変更する能力を与えます。

これを行うために、プレイヤーに<DynamicLink href="/start/input">Pixi’VNの機能を使用して入力ボックスを完成させる</DynamicLink>ように依頼します。

入力値を取得した後、得られた値を使用して<DynamicLink href="/start/character#edit">キャラクター名を設定する</DynamicLink>ことができます。
...
今、私たちは対話の中で<DynamicLink href="/start/character#use">キャラクター名を使用できます</DynamicLink>。
...
視覚小説では、現在の対話にテキストを貼り付けることがよく役立ちます。 例えば、会話を一時停止し、その後の`step`で続けることができます。 これを行うために、<DynamicLink href="/start/dialogue#dialogue-glue">接着機能</DynamicLink>を使用できます。
...
アセット（画像、GIF、動画など）をロードして操作するには `Assets`を使用する必要があります。 `Assets`は多くの機能を持つクラスで、PixiJSライブラリから来ています。詳細については[こちら](https://pixijs.com/8.x/guides/components/assets)をお読みください。

</Callout>

最初のステップの一つは、視覚小説のアセットを保存する場所を選ぶことです。 この場合、アセットはFirebaseストレージ（ホスティングサービス）に保存します。 お好きなホスティングサービスを使用することもできますし、プロジェクト内にアセットをローカルに保存することもできます。詳しくは<DynamicLink href="/start/assets">こちら</DynamicLink>をご覧ください。

アセットを使用する前に、<DynamicLink href="/start/assets-management#initialize-the-asset-matrix-at-project-start">アセットマトリックスを初期化する</DynamicLink>ことが強く推奨されます。

デフォルトでは、`assets/manifest.ts`ファイルに示すように、`onLoadingLabel`のすべてのテンプレートは、現在の`label`のIDに等しいエイリアスを持つ「バンドルアセット」をバックグラウンドでロードしようとします。 したがって、`manifest`に各`label`に対してエイリアスが`label`のIDと等しく、その`label`で使用される画像を含む「バンドルアセット」を追加することが推奨されます。
...
さあ、視覚的な部分についても考える時です。 背景とキャラクタースプライトを視覚小説のキャンバスに追加します。

**スプライトとは何ですか？** コンピュータグラフィックスにおいて、スプライトは大きなシーンに統合された二次元ビットマップであり、最も多くは2Dビデオゲームで使用されます。

私たちの場合、キャラクタースプライトは体、目、口の3つの画像で構成されています。 次に、<DynamicLink href="/start/canvas-image-container">ImageContainer</DynamicLink>を使用してキャラクターを構成します。 キャンバスコンポーネントの追加方法については、<DynamicLink href="/start/canvas-components">このドキュメント</DynamicLink>でさらに情報を見つけることができます。
...
私たちの場合、ゲーム画像はホスティングサービス（Firebase）に保存しました。 このため、アセットの読み込みはタイムリーではありません。

プレイヤーが過剰な読み込みを感じないように、ゲームの特定のフェーズにそれらをグループ化する必要があります。 私の場合、`label`の最初に最も使用される画像をロードします。

読み込みを管理する方法については、<DynamicLink href="/start/assets-management">こちら</DynamicLink>で詳細情報を見つけることができます。
...
視覚小説をよりダイナミックにするために、画像を表示するためのトランジションを使用できます。 トランジションの使用に関する詳細情報は<DynamicLink href="/start/canvas-transition">こちら</DynamicLink>で見つけることができます。
