...
ゲームの冒頭部分とします。 その後、このビジュアルノベルで続いていく <DynamicLink href="/start/dialogue">dialogue</DynamicLink> を記述します。
...
非常に長い `labels` を作成することは推奨されません（線形のビジュアルノベルであっても同様です）。その代わりに、小さな `labels` を複数作成し、必要に応じて <DynamicLink href="/start/labels-flow">ナレーションのフロー制御機能</DynamicLink> 
を使用して呼び出す方法が一般的です。
...
次に、プレイヤーに物語の次のパートへ進むかどうかを尋ねます。

これを行うために、<DynamicLink href="/start/choices">選択肢メニュー</DynamicLink> を使用します。
...
これを実現するために、 <DynamicLink href="/start/input">Pixi’VN の入力ボックス機能</DynamicLink> を使用します。

プレイヤーが入力した値を取得した後、 <DynamicLink href="/start/character#edit">キャラクター名を編集</DynamicLink> できます。
...
その後、更新されたキャラクター名を <DynamicLink href="/start/character#use">dialogue 内で使用</DynamicLink> できます。
