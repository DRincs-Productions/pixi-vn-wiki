...
このチュートリアルでは、初めてのビジュアルノベルを作成するプロセスを説明します。

**ビジュアルノベルとは？** ビジュアルノベル（VN）はデジタルインタラクティブフィクションの一種です。 ビジュアルノベルはビデオゲームと関連付けられることが多いものの、必ずしもゲームとして扱われるとは限りません。 文章による物語、静止画またはアニメーション、そして一定のインタラクティビティを組み合わせた形式です。 英語では _novel game_ と呼ばれることもあり、日本語の「ノベルゲーム」を由来とした表現です。

学習目的として、このガイドでは Pixi’VN を使ってビジュアルノベル 
[Breakdown](https://joshpowlison.itch.io/breakdown) を再現します。 Breakdown はビジュアルノベルに必要な要素を備えた短い物語です。 Breakdown はビジュアルノベルに必要な要素を備えた短い物語です。 
作者から教育目的で使用する許可を得ています ❤️。

Pixi’VN では、<DynamicLink href="/start/narration">利用可能なナラティブ言語</DynamicLink> を 1 つ以上選んで物語を書くことができます。 このガイドでは、開発の各ステップでそれぞれの言語による例を示します。

## 新しいプロジェクトを作成する

最初のステップは、新しいプロジェクトを作成することです。 テンプレートからプロジェクトを作成する方法については <DynamicLink href="/start#project-initialization">こちら</DynamicLink> を参照してください。 このガイドではテンプレート **"Visual Novel - React"** を使用します。

`Visual Novel -> React`

プロジェクト作成後、プロジェクトルートにある `README.md` を必ず確認してください。 このファイルにはプロジェクトの使用方法に関する重要な情報が含まれています。

本例では、プロジェクトを開始するには次のコマンドを実行するだけで十分です：
...
次に、この物語で使用するキャラクターを定義します。 使用するキャラクターは `/values/characters.ts` に記述します。 キャラクターの作成や利用方法についての詳細は <DynamicLink href="/start/character">Characters</DynamicLink> を参照してください

`mc` とは何でしょうか？ `mc` は “Main Character”（主人公）の略で、ビジュアルノベルで一般的に使われる命名です。 ビジュアルノベルでは、主人公の名前として mc を使用するのが一般的です。
...
すべてのテンプレートは  <DynamicLink href="/start/markup-markdown">Markdown</DynamicLink> と  
<DynamicLink href="/start/markup-tailwindcss">Tailwind CSS</DynamicLink> をサポートしているため、  
これらを使ってナラティブを書くことができます。

</Callout>

次に、ビジュアルノベルの <DynamicLink href="/start/narration">ナレーション</DynamicLink> の初稿を書き始めます。
最初の <DynamicLink href="/start/labels">`label`</DynamicLink> を `start` として作成し、 
ゲームの冒頭部分とします。 その後、このビジュアルノベルで続いていく 
<DynamicLink href="/start/dialogue">dialogue</DynamicLink> を記述します。
