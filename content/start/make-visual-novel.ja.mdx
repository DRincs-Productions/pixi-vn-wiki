...
description: Pixi’VN を使ってビジュアルノベルを作成するためのステップバイステップガイド（プロジェクトセットアップ、ナラティブ、アセット、インタラクティビティを含む）。
---

import { TSVisualNovelExample } from "@/components/vn-examples";
import { InkVisualNovelExample } from "@/components/vn-ink-examples";

このチュートリアルでは、初めてのビジュアルノベルを作成するための手順を順を追って説明します。

**ビジュアルノベルとは？** ビジュアルノベル（VN）はデジタルインタラクティブフィクションの一種です。 ビジュアルノベルはゲームと関連付けられることが多いものの、必ずしもゲームとして分類されるわけではありません。 文章による物語、静止画またはアニメーション、そしてさまざまなインタラクティブ要素を組み合わせた表現形式です。 日本では「ノベルゲーム」という和製英語が一般的で、英語では _novel game_ と表記される場合もあります。
