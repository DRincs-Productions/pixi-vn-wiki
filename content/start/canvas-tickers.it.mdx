...
You can use the `startOnlyIfHaveTexture` property to start the animation only if the canvas component have a texture not empty. This is actually during the [creation of a transition](/start/canvas-transition#create-your-own-transition).

### Alias to remove after property

You can use the `aliasToRemoveAfter` property to remove the canvas component after the effect. This is actually during the [creation of a transition](/start/canvas-transition#create-your-own-transition).
