...
이 예시에서는 이동 인/아웃 트랜지션을 사용하여 `james`가 캔버스 외부에서 장면 안으로 들어오도록 합니다. In *ink*, this is done by appending `with movein` (and its parameters) to the `# show` command, while in JS/TS you can use the `transitions.moveIn`/`transitions.moveOut` functions.
...
To take `steph` out/in, I will use the `transitions.moveOut` and `transitions.moveIn` functions. 미러 효과를 위해서는 `canvas.animate` 함수를 사용하겠습니다.
...
다음 내러티브 단계 (`step`)가 실행되기 전에 애니메이션이 완료되도록 `completeOnContinue: true` 옵션을 사용합니다. For `transitions.moveIn`, being a transition, `completeOnContinue` is true by default.
