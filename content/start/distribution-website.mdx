...
Your Pixi’VN game can be distributed as a website, allowing players to access it through their web browsers. This is a great way to share your game with a wider audience without requiring them to download or install anything.
can be distributed as a website. The game can be played on the browser and can be accessed by anyone with an internet connection. To do this you need to [host the game on a server](#hosting-and-deploying).
...
*"Detected an existing Vite codebase in the current directory, should we use this?"* Enter `no`.

*"Do you want to use a web framework? (experimental)"* Enter `no`.
...
*"Set up automatic builds and deploys with GitHub?"* Enter `no`. (This is optional, but you can set it up later if you want to.)
