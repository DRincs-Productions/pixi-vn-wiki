...
export default async function showWithDissolve(
    ...
    props: FadeAlphaTickerProps && {
        forceCompleteBeforeNext?: boolean // [!code ++]
    } = {},
    priority?: UPDATE_PRIORITY,
): Promise<string[] | undefined> {
    let forceCompleteBeforeNext = props.forceCompleteBeforeNext ?? true // [!code ++]
    ...
    if (id) { // [!code ++]
        forceCompleteBeforeNext && canvas.completeTickerOnStepEnd({ id: id }) // [!code ++]
    } // [!code ++]
