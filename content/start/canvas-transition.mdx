...
* `options` (Optional): The animation options. It corresponds to the options of the <DynamicLink href="/start/canvas-motion">`options` of animate function</DynamicLink>.
...
* `image` (Optional): The image to show. If you don't provide the URL, then the alias is used as the URL. It can be:
  ...
  * a `{ value: string[], options: ImageContainerOptions }`, where `value` is the array of URL/paths and `options` is the the options of the <DynamicLink href="/start/canvas-images">ImageContainer</DynamicLink>.
  * a <DynamicLink href="/start/canvas-components">canvas component</DynamicLink>.
* `options` (Optional): The animation options. It corresponds to the options of the <DynamicLink href="/start/canvas-motion">`options` of animate function</DynamicLink>. Additionally, it can have the following properties:
...
Create your own transition is very simple, you can use <DynamicLink href="/start/canvas-motion">`canvas.animate`</DynamicLink> to create your own transition.

To better understand how to create one, a simplified version of [`showWithDissolve`](#dissolve) will be left as an example below.
...
To remove the previous component when the new component transition is complete, you can use the <DynamicLink href="/start/canvas-motion">`aliasToRemoveAfter` property</DynamicLink>.
...
To remove the previous component with a transition, you can run a another animation with the <DynamicLink href="/start/canvas-motion">`canvas.animate`</DynamicLink> function, and use the <DynamicLink href="/start/canvas-motion">`tickerIdToResume` property</DynamicLink>
