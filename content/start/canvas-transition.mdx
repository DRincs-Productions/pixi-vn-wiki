...
export default async function showWithDissolve(
    ...
    // edit the zIndex of the old component to make sure the new component is above the old component // [!code ++]
    const zIndex = oldComponent?.parent?.getChildIndex(oldComponent); // [!code ++]
    oldComponent?.parent?.setChildIndex(oldComponent, zIndex ? zIndex - 0.1 : 0); // [!code ++]
