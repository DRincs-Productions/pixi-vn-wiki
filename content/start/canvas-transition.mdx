...
* `image` (Optional): The image to show. If you don't provide the URL, then the alias is used as the URL. It can be:
  * a URL/path. In this case, if URL/path is a video will be added a <DynamicLink href="/start/canvas-videos.md">VideoSprite</DynamicLink>, else a <DynamicLink href="/start/canvas-images">ImageSprite</DynamicLink>.
  * a array of URL/paths. In this case, will be added a <DynamicLink href="/start/canvas-images">ImageContainer</DynamicLink>.
  * a `{ value: string, options: ImageSpriteOptions }`, where `value` is the URL/path and `options` is the the options of the <DynamicLink href="/start/canvas-images.md">ImageSprite</DynamicLink>. In this case, if URL/path is a video will be added a <DynamicLink href="/start/canvas-videos.md">VideoSprite</DynamicLink>, else a <DynamicLink href="/start/canvas-images">ImageSprite</DynamicLink>.
...
This transition has been created with the [`ZoomTicker`](/start/canvas-animations-effects#zoom).
...
* `props` (Optional): The properties of the effect. It combines the properties of the [Zoom effect](/start/canvas-animations-effects#zoom) with following properties:
...
To do this, you can use the [`canvas.completeTickerOnStepEnd` function](/start/canvas-tickers-functions#force-completion-of-the-transition-at-the-end-of-the-step).
