...
The game flow is managed by functions such as `callLabel`, `jumpLabel`, `goNext`, `goBack`, and `closeLabel`, all available in the `narration` object.
...
To call a label, use the `narration.callLabel` function. This function has two parameters:

* `label`: the label to be called
* `props`: the properties to pass to the label. The interface corresponds to <DynamicLink href="/start/labels#override-steplabelprops">`StepLabelProps`</DynamicLink>.
...
To jump to a label, use the `narration.jumpLabel` function with two parameters:
...
To execute the next step, use the `narration.goNext()` function. This function takes one parameter:
...
You must pass a function `navigate: (path: string) => void` as a parameter. This function will be called with the <DynamicLink href="/start/interface#what-is-the-url-path-and-routes">URL Path or Route</DynamicLink> of the previous step, so you can <DynamicLink href="/start/interface-navigate">navigate to the previous UI screen</DynamicLink>.
