...
description: Explains how to use boolean flags in Pixi'VN to control game flow and manage state efficiently.
---
...
This mechanic has much less impact on save size than directly <DynamicLink href="/start/storage#set-a-variable-in-the-game-storage">saving a boolean in Game Storage</DynamicLink>.

## Set

To set a flag, use the `storage.setFlag` function. Parameters:
...
To get a flag, use the `storage.getFlag` function. Parameter:
