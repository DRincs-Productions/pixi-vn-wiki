...
=== start ===
# show spine boy skeleton spineboySkeleton atlas spineboyAtlas x 0.5 y 1 animation idle
-> DONE
```
...
A Spine model can have multiple skins, and you can switch between them at runtime by using the `setSkin` function.
...
```ink tab="ink" title="ink/start.ink" groupId="narrative_language"
=== start ===
# show spine goblin skeleton goblinsSkeleton atlas goblinsAtlas skin goblin x 0.5 y 1 animation walk
# pause
# change spine goblin skin goblingirl
# pause
-> DONE
```
...
There are two functions you can use to play a Spine animation: `setAnimation` and `addAnimation`.

- `setAnimation` immediately replaces whatever animation is currently playing on the track with the new one.
- `addAnimation` queues the new animation so that it starts only after the current animation on the track finishes, without interrupting it.
...
# show spine boy skeleton spineboySkeleton atlas spineboyAtlas x 0.5 y 1 animation idle
# pause
# animate spine boy walk loop true
# pause
-> DONE
```
...
You can combine Spine animations with Pixi’VN's classic motion-based animations (`canvas.animate`), as shown in the example below.
...
It is also possible to create animation sequences by using the `playSequence` function.
...
You can remove Spine animations using two methods: `clearTrack`, which clears the animation on a single track, and `clearTracks`, which clears the animations on all tracks.
