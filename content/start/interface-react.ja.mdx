...
icon: /react.svg
title: React UI
description: Pixi’VNをReactと統合する方法、セットアップ、テンプレート、および最適なUIライブラリを含む。 Create a React visual novel or 2D game.
---

**Reactとは何ですか？** Reactは、ウェブサイトやアプリなどのユーザーインターフェース（UI）を構築するためのJavaScriptライブラリです。 無料でオープンソースであり、Meta（旧Facebook）と大規模なコミュニティによって維持されています。

[Reactのウェブサイト](https://react.dev/)でReactについてさらに学ぶことができます。

## Pixi’VNをReactアプリケーションに追加する方法

<Callout title="React template" type="info">

Reactを使用したPixi’VNのテンプレートが利用可能です。 それらについては、<DynamicLink href="/start#project-initialization">テンプレート</DynamicLink>のセクションでさらに読むことができます。

既存のReactアプリケーションにPixi’VNを追加したい場合は、以下の手順に従ってください。

</Callout>

まず、Reactアプリケーションと<DynamicLink href="/start#installation">pixi-vnをインストール</DynamicLink>する必要があります。 新しいReactアプリケーションを作成するには、[Vite](https://vite.dev/guide/#scaffolding-your-first-vite-project)を使用することをお勧めします。
...
## 最高のReactコンポーネントライブラリ

美しいユーザーインターフェースを作成するには、いくつかの優れたReact UIコンポーネントライブラリを使用できます。
