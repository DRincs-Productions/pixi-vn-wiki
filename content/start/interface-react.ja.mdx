---
title: React UI
description: Pixi’VNをReactと統合する方法（セットアップ、テンプレート、最適なUIライブラリを含む）。 Reactでビジュアルノベルやストーリー主導のゲームを作成しましょう。
---

Pixi’VNは**ビジュアルノベルReact**エンジンとして優れた性能を発揮し、Pixi’VNがナレーションとキャンバスを処理する一方で、Reactコンポーネントを使ってビジュアルノベルやストーリー重視のゲームのUIを構築できます。
...
Reactについては、[Reactのウェブサイト](https://react.dev/)で詳しく知ることができます。

## Pixi’VNをReactアプリケーションに追加する方法

:::tip[テンプレート]

Reactを使用したPixi’VNのテンプレートが用意されています。 詳細については、<DynamicLink href="/start#project-initialization">テンプレート</DynamicLink>セクションをご覧ください。If you want to add Pixi’VN to an existing React application, follow the steps below.

If you want to add Pixi’VN to an existing React application, follow the steps below.

:::

まず、Reactアプリケーションが必要であり、<DynamicLink href="/start#installation">pixi-vnをインストール</DynamicLink>する必要があります。 新しいReactアプリケーションを作成するには、[Vite](https://vite.dev/guide/#scaffolding-your-first-vite-project)を使用することをお勧めします。
