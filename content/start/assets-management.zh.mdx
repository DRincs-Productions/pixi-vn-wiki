...
To load and manipulate assets (images, gifs, videos, etc.) 您将需要使用 `Assets`。 `Assets` is a class with many features and comes from the PixiJS library. For more information, read [here](https://pixijs.com/8.x/guides/components/assets).
