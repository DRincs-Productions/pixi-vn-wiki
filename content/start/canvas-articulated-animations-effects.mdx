...
* `props`: the properties of the shake effect. The properties are:
  ...
  * `startOnlyIfHaveTexture` (Optional): is a boolean value. If true, the animation will alter the properties only if the canvas component has a non-empty texture, otherwise it will run on empty. You can read more about it [here](/start/canvas-tickers#start-only-if-have-texture-property).
