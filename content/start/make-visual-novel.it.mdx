...
Per aggiungere suoni e musica alla tua visual novel, puoi utilizzare l'utilità `sound`. Puoi trovare ulteriori informazioni su come utilizzarle <DynamicLink href="/start/sound">qui</DynamicLink>.

Il primo passaggio consiste nel definire le risorse audio in `manifest`. Quindi, puoi caricarle in background all'avvio del gioco, così che saranno disponibili quando necessarie, senza interrompere l'avvio del gioco.
...
Inoltre, puoi definire i canali audio per gestire diversi tipi di suono (es., BGM, SFX) e controllarne il volume, metterli in pausa, riprenderne la riproduzione, etc. Si consiglia di definire i canali all'inizio del gioco, ad esempio nella richiamata `then` della funzione `Game.init`.
