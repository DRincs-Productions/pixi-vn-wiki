...
Non è consigliabile creare `labels` molto lunghe (anche per visual novel lineari), ma è consigliabile creare più `labels` piccole e "chiamarle" quando necessario con le <DynamicLink href="/start/labels-flow">funzionalità di controllo del flusso di gioco</DynamicLink> (<DynamicLink href="/ink/ink-label">_ink_ knot (o label)</DynamicLink>).
