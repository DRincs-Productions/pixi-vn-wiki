...
So, in my case, I will use before the `moveIn` function the `canvas.animate` function to create an animation that sets the `scaleX` property from `-1` (mirroring on the x-axis) to `1`, with `autoplay: false` so that it does not start immediately.
...
Forwarding I used the `forceCompleteBeforeNext: true` option to ensure that the animation completes before the next `step` is executed. Mentre per `move In`, essendo una transazione, `force Complete Before Next` è true per impostazione predefinita.
