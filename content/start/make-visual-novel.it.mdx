...
Poiché Pixi’VN ti dà la possibilità di scrivere la tua narrazione scegliendo un o più <DynamicLink href="/start/narration">linguaggio narrative disponibili</DynamicLink>, verranno realizzati esempi per ciascun linguaggio attualmente disponibile in ogni `step` di sviluppo.

## Crea un nuovo progetto

Il primo `step` è creare un nuovo progetto. Puoi trovare maggiori informazioni su come creare un nuovo progetto partendo da un template <DynamicLink href="/start/getting-started#project-initialization">qui</DynamicLink>. Utilizzeremo il template "Visual Novel - React".
...
Creeremo la prima <DynamicLink href="/start/labels">`label`</DynamicLink> chiamata `start`, che sarà l'inizio del gioco.
...
Non è consigliabile creare `labels` molto lunghe (anche per visual novel lineari), ma è consigliabile creare più `labels` piccole e "chiamarle" quando necessario con le <DynamicLink href="/start/labels-flow.md">funzionalità di controllo del flusso di gioco</DynamicLink> (<DynamicLink href="/ink/ink-label">_ink_ knot (o label)</DynamicLink>).
