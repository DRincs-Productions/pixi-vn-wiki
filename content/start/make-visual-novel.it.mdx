...
Il primo passo è creare un nuovo progetto. Puoi trovare maggiori informazioni su come creare un nuovo progetto partendo da un template <DynamicLink href="/start/getting-started#project-initialization">qui</DynamicLink>. Utilizzeremo il template "Visual Novel - React".
...
Non è consigliabile creare label molto lunghe (anche per visual novel lineari), ma è consigliabile creare più label piccole e "chiamarle" quando necessario con le <DynamicLink href="/start/labels-flow.md">funzionalità di controllo del flusso di gioco</DynamicLink> (<DynamicLink href="/ink/ink-label">_ink_ knot (o label)</DynamicLink>).
...
Ora posso chiamare questa etichetta `animation_01` dall'etichetta principale `start`. (Come spiegato <DynamicLink href="/ink/ink-label#use-the-call-script">qui</DynamicLink> da _ink_ posso chiamare le etichette scritte in ts e viceversa.)
