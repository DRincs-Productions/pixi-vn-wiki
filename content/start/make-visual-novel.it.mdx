...
Per fare questo, imposteremo [`narration.choices`](/jsdoc/pixi-vn/index/interfaces/NarrationChoicesInterface#list) su un array di opzioni di scelta create con [`newChoiceOption`](/jsdoc/pixi-vn/index/functions/newChoiceOption) (per saltare a o chiamare una `label`) e [`newCloseChoiceOption`](/jsdoc/pixi-vn/index/functions/newCloseChoiceOption) (per chiudere il menu di scelta senza navigare da nessuna parte).
...
In questo esempio, usiamo la transizione move in/out per far entrare `james` nella scena dall'esterno del canvas. In *ink*, this is done by appending `with movein` (and its parameters) to the `# show` command, while in JS/TS you can use the `transitions.moveIn`/`transitions.moveOut` functions.
...
To take `steph` out/in, I will use the `transitions.moveOut` and `transitions.moveIn` functions. Per l'effetto specchio, userò la funzione `canvas.animate`.
...
Uso l'opzione `completeOnContinue: true` per assicurarmi che l'animazione si completi prima che venga eseguito lo `step` successivo. For `transitions.moveIn`, being a transition, `completeOnContinue` is true by default.
