...
Il primo `step` è creare un nuovo progetto. You can find more information on how to create a new project starting from a template <DynamicLink href="/start#project-initialization">here</DynamicLink>. Utilizzeremo il template "Visual Novel - React".
...
Non è consigliabile creare `labels` molto lunghe (anche per visual novel lineari), ma è consigliabile creare più `labels` piccole e "chiamarle" quando necessario con le <DynamicLink href="/start/labels-flow">funzionalità di controllo del flusso di narrazione</DynamicLink> (<DynamicLink href="/ink/ink-label">_ink_ knot (o label)</DynamicLink>).
