...
Nel nostro caso gli sprite dei personaggi sono composti da 3 immagini: il corpo, gli occhi e la bocca. Quindi utilizziamo <DynamicLink href="/start/canvas-image-container">ImageContainer</DynamicLink> per comporre il personaggio. You can find more information on how to add canvas components in <DynamicLink href="/start/canvas-components">this documentation</DynamicLink>.
...
Per rendere più dinamica la visual novel, è possibile utilizzare le transizioni per mostrare le immagini. You can find more information about using transitions <DynamicLink href="/start/canvas-transition">here</DynamicLink>.
...
Per rendere più dinamica la visual novel, è possibile utilizzare le animazioni. You can find more information about how to use animations <DynamicLink href="/start/canvas-animations-effects">here</DynamicLink>.
...
Per far entrare/uscire `steph` userò le funzioni `moveOut` e `moveIn`. For the mirror effect I will use the `canvas.animate` function.
...
Then I will use the `moveIn` function to move `steph` into the scene, and I will pass the `tickerId` of the animation to the `moveIn` function so that it can be resumed after the transition is complete.
