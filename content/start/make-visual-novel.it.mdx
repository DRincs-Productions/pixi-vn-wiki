...
Per rendere più dinamica la visual novel, è possibile utilizzare le animazioni. Puoi trovare maggiori informazioni su come utilizzare le animazioni <DynamicLink href="/start/canvas-animations-effects">qui</DynamicLink>.
...
Per far entrare/uscire `steph` userò le funzioni `moveOut` e `moveIn`. Per l'effetto specchio userò la funzione `canvas.animate`.

So, in my case, I will use before the `moveIn` function the `canvas.animate` function to create an animation that sets the `scaleX` property to `-1` (mirroring on the x-axis) and then set it back to `1`, with `autoplay: false` so that it does not start immediately.

Quindi userò la funzione `moveIn` per spostare `steph` nella scena e passerò il `tickerId` dell'animazione alla funzione `moveIn` in modo che possa essere ripresa una volta completata la transizione.

Forwarding I used the `forceCompleteBeforeNext: true` option to ensure that the animation completes before the next step in the label is executed. While for `move In`, being a transaction, `force Complete Before Next` is true by default.
...
Ora posso chiamare questa `label` `animation_01` dall'`label` principale `start`. (Come spiegato <DynamicLink href="/ink/ink-label#use-the-call-script">qui</DynamicLink> da _ink_ è possibile chiamare `labels` scritte in ts e viceversa.)
