...
- `videoUrl` (Optional): The URL or path. 如果你已经初始化了<DynamicLink href="/start/assets-management#initialize-the-asset-matrix-at-project-start">资源矩阵</DynamicLink>，你可以使用纹理的别名。
...
This function returns a `VideoSprite` that you can use to manipulate the component. 该函数具有以下参数：

- `alias`: The <DynamicLink href="/start/canvas-alias">alias</DynamicLink> to identify the component.
- `videoUrl` (Optional): The URL or path. 如果你已经初始化了<DynamicLink href="/start/assets-management#initialize-the-asset-matrix-at-project-start">资源矩阵</DynamicLink>，你可以使用纹理的别名。 If you don't provide the URL, then the alias is used as the URL.
- `options` (Optional): The options for the component.
