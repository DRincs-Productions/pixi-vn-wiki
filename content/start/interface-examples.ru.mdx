---
title: Примеры UI-экранов
description: Предоставляет примеры UI-экранов для Pixi’VN, показывая компоненты React и Vue, а также демонстрируя хуки (например, useQueryDialogue, useQueryChoiceMenuOptions, useQueryCanGoNext) и интеграцию с повествованием для выбора, вперёд/назад, запросов ввода и истории.
---

:::tip[ИИ]

Вы можете использовать кнопку выше, чтобы создать собственный UI-экран с помощью ИИ.

:::

:::tip

В этих примерах используются простые UI-примитивы (`Button`, `Card`, `Avatar`, `Dialog`, `Input`, ...) импортированные из папки `components/ui`, аналогично тому, что предоставляют [shadcn/ui](https://ui.shadcn.com) (React) или [shadcn-vue](https://www.shadcn-vue.com). Замените их эквивалентными компонентами из вашей собственной UI-библиотеки.

:::

<Accordions>

<Accordion title="UI-экран диалога повествования" id="narrative-dialogue">
...
<Accordion title="UI-экран меню выбора" id="choice-menu">
...
<Accordion title="Продолжение по клику на экран" id="go-next">

Вместо отдельной кнопки «далее» оберните экран повествования в оверлей, который продвигает повествование при клике игрока в любом месте экрана.
...
Оберните этим экран диалога повествования, чтобы клик в любом месте экрана продолжал повествование (такие элементы, как меню выбора, кнопка назад и диалог ввода, вызывают `stopPropagation()`, чтобы взаимодействие с ними не запускало также «далее»):
...
<Accordion title="Кнопка назад" id="go-back">
...
<Accordion title="UI-диалог запроса ввода" id="input-prompt">
