...
* The game ends only when all `steps` are completed. At that point, the <DynamicLink href="/start/other-narrative-features#dynamic-step-lis">Game.onEnd</DynamicLink> function is triggered.
...
* `steps`: An array of functions to be executed in order. If you want create a dynamic array, you can read <DynamicLink href="/start/labels-advanced#onstepstart">here</DynamicLink>.
