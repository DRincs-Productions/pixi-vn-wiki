...
But **in case the assets are not saved locally**, but in an ["assets hosting"](/start/assets.md#assets-hosting) their loading may take some time. This means that starting a step may not be timely. So the player after starting the execution of the [next step](/start/labels-flow#next-step) (for example with the "next" button) may have to wait some time to be able to "view" the changes and be able to run another step.
...
It is possible to load assets as project starts in the background, giving the player the ability to interact with the project while it loads. It is recommended not to exceed 2GB.

To do this, you can use the `backgroundLoad` function when the project starts. You can learn more about the `Assets.load` function [here](https://pixijs.com/8.x/guides/components/assets).
