...
To do this, you must use the `Assets.load` function and wait for it to finish (with `wait`) when the project starts. The `Assets.load` function is asynchronous and returns a promise that will be resolved when all the assets are loaded. The `Assets.load` function requires a string that is the alias of the asset to be loaded.
