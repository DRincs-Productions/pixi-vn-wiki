...
In the `defineAssets` function we will use `Assets` (e.g. `Assets.add`, `Assets.addBundle` and `Assets.init`) to assign an alias to one or more URLs. You can find more information about them [here](https://pixijs.com/8.x/guides/components/assets) to assign an alias to each asset.
