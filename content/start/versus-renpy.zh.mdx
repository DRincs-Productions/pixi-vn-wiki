...
在 Ren'Py 中，要创建带有复杂机制/动画的小游戏，你需要使用[` 创作者定义的可视组件（CDD）`](https://doc.renpy.cn/zh-CN/cdd.html)。这是很有必要的，否则可能会出现性能问题。通过CDD，你可以在渲染循环中创建/控制一个或多个图形元素。但CDD不够直观，使用复杂，示例稀少，开发者也不算多。事实上，绝大多数Ren'Py小游戏并没有复杂的逻辑与动画。
