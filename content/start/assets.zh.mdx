...
要在本地保存和使用资源，你可以使用任何文件夹——没有限制。 但建议使用 `src/assets` 文件夹。 在此文件夹中，您可以创建子文件夹以更好地组织资源。
...
在开发阶段（`vite dev`），它会监视 `src/assets` 文件夹的变化并即时处理。 在生产构建阶段（`vite build`），它会在打包前执行完整的优化流程。
...
处理完成后，它会在 `public/assets` 中生成优化后的文件，并生成 `src/assets/manifest.gen.json` 文件，其中包含所有资源包及其路径的列表。 这些输出随后会被 PixiJS 用来高效地加载资源。
