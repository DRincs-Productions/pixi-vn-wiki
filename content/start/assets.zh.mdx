...
你可以将你的资源保存在线上。 如果您想要节省电脑空间的话，这是一个不错的选择。 你可以使用任何允许上传文件并生成公开URL（需启用 CORS）的云服务。
...
你可以使用Github来托管你的资源。 可以在项目中使用文件的原始链接。 链接格式如下：
...
图片托管服务是一种允许上传图片的服务。 有许多网站可以免费上传图片，例如[imgbb](https://imgbb.com/)、[imgix](https://www.imgix.com/)、[imgur](https://imgur.com/)（译者注：由于众所周知的原因，后两者已经被河蟹了）。 你可以在项目中使用这些图片的链接。
...
- ![firebase-text](/firebase.svg)**Firebase Storage**是一项非常易于使用的云服务。 Firebase提供两种套餐：Spark（免费）和 Blaze（按需付费）。 你可以在[这里](https://firebase.google.com/pricing)查看更多信息。**解决Firebase Storage的CORS问题**：
