...
要加载和操作资源（如图片、动图、视频等）， 需要使用 `Assets`。 `Assets`来自PixiJS库，是一个具有丰富功能的类。 如要了解更多信息，请参阅[此处](https://pixijs.com/8.x/guides/components/assets)。 我们强烈建议你阅读这份文档，以便于更好地<DynamicLink href="/start/assets-management">组织资源的上传工作</DynamicLink>。

你主要有两种存储资源的选择：本地或在线。

## 本地资源

要在本地保存与使用资源，你可以使用任意文件夹，没有限制。 不过，建议使用`assets`文件夹。 在该文件夹内，你可以创建子文件夹，来更好地组织资源。
