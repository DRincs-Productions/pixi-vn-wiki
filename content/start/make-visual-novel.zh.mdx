...
现在我们可以开始编写视觉小说<DynamicLink href="/start/narration">叙事</DynamicLink>的"初稿"了。我们将创建第一个名为 `start` 的<DynamicLink href="/start/labels">叙事节点（`label`）</DynamicLink>，它将是游戏的开始。之后我们可以编写视觉小说中接下来的<DynamicLink href="/start/dialogue">对话</DynamicLink>。
...
为此，我们将把 [`narration.choices`](/jsdoc/pixi-vn/index/interfaces/NarrationChoicesInterface#list) 设置为一个由 [`newChoiceOption`](/jsdoc/pixi-vn/index/functions/newChoiceOption)（用于跳转或调用某个叙事节点（`label`））和 [`newCloseChoiceOption`](/jsdoc/pixi-vn/index/functions/newCloseChoiceOption)（用于关闭选择菜单而不跳转到任何地方）创建的选择项数组。
...
现在我将赋予玩家更改 `mc` 名字的能力。为此，我将要求玩家<DynamicLink href="/start/input">使用 Pixi’VN 的功能填写一个输入框</DynamicLink>。
...
在这个示例中，我们使用移入/移出（move in/out）过渡效果，让 `james` 从画布外部进入场景。 In *ink*, this is done by appending `with movein` (and its parameters) to the `# show` command, while in JS/TS you can use the `transitions.moveIn`/`transitions.moveOut` functions.
...
To take `steph` out/in, I will use the `transitions.moveOut` and `transitions.moveIn` functions. 对于镜像效果，我将使用 `canvas.animate` 函数。
...
我使用 `completeOnContinue: true` 选项来确保动画在下一个叙事步骤（`step`）执行之前完成。 For `transitions.moveIn`, being a transition, `completeOnContinue` is true by default.
