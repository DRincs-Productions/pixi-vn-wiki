...
为此，我们将把 [`narration.choices`](/jsdoc/pixi-vn/index/interfaces/NarrationManagerInterface#choices) 设置为一个由 [`newChoiceOption`](/jsdoc/pixi-vn/index/functions/newChoiceOption)（用于跳转或调用某个叙事节点（`label`））和 [`newCloseChoiceOption`](/jsdoc/pixi-vn/index/functions/newCloseChoiceOption)（用于关闭选择菜单而不跳转到任何地方）创建的选择项数组。
