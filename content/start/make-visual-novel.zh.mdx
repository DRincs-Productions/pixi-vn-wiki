...
我们将创建第一个名为 `<DynamicLink href="/start/labels">`label`</DynamicLink>` 的 `start`，这将是游戏的开始。 After that we can write the <DynamicLink href="/start/dialogue">dialogues</DynamicLink> that will follow in our visual novel.
