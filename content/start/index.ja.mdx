...
- `options`: Equivalent to the options you can use when initializing a [PixiJS Application](https://pixijs.com/8.x/guides/components/application). 此外，还支持以下选项：
  - `id`：分配给 canvas 元素的 ID。
  - `resizeMode`：使用的调整大小模式。 可能的值为：
    - `none`：不调整大小。
    - `contain`：canvas 会调整大小以适应父元素，同时保持其宽高比。 （默认）
