---
title: UI 界面示例
description: 为 Pixi’VN 提供 UI 界面示例，展示 React 和 Vue 组件，并演示 Hook（例如 useQueryDialogue、useQueryChoiceMenuOptions、useQueryCanGoNext）以及与叙事的集成，涵盖选项、前进/后退、输入提示和历史记录。
---

:::tip[AI]

你可以使用上方的按钮，用 AI 创建你自己的 UI 界面。

:::

:::tip

这些示例使用了简单的 UI 基础组件（`Button`、`Card`、`Avatar`、`Dialog`、`Input` 等） 它们从 `components/ui` 文件夹导入，类似于 [shadcn/ui](https://ui.shadcn.com)（React）或 [shadcn-vue](https://www.shadcn-vue.com) 提供的组件。 请将它们替换为你自己 UI 组件库中的等效组件。

:::

<Accordions>

<Accordion title="叙事对话 UI 界面" id="narrative-dialogue">
...
<Accordion title="选项菜单 UI 界面" id="choice-menu">
...
<Accordion title="点击屏幕继续" id="go-next">

不使用专门的“下一步”按钮，而是将叙事界面包裹在一个覆盖层中，玩家点击该覆盖层的任意位置都会推进叙事。
...
用它包裹叙事对话界面，这样点击屏幕上的任意位置都会继续叙事（选项菜单、返回按钮和输入对话框等元素会调用 `stopPropagation()`，因此与它们交互不会同时触发“下一步”）：
...
<Accordion title="返回按钮" id="go-back">
...
<Accordion title="输入提示对话框 UI" id="input-prompt">
