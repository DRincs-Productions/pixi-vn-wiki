...
* `options` (Optional): [opzioni `motion`](https://motion.dev/docs/animate#options) per l'animazione, inclusi durata, `easing` e ticker. Le seguenti opzioni estendono quelle di `motion`:
  ...
  * `completeOnContinue` (Opzionale): Un valore booleano che indica se l'animazione deve essere completata prima del successivo `step` del gioco. Se `true`, il gioco forzerà il completamento dell'animazione prima di procedere.
