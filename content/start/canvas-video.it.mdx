...
Il componente `VideoSprite` estende il componente <DynamicLink href="/start/canvas-image">`ImageSprite`</DynamicLink>, così puoi utilizzare tutti i metodi e le proprietà di `ImageSprite`. Viene utilizzato per visualizzare un singolo video sul canvas.
