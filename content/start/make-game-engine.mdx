...
The first step is to create a new project. You can find more information on how to create a new project starting from a template [here](/start/getting-started#project-initialization). The "Game Engine" template is a simple project that contains the basic structure of a game engine. By default, it exports the basic functionality of pixi-vn while removing anything unused from the package.
...
Since it has been handled in a generic way, you can implement your own [`Label`](/start/labels#label) (the key element of narration) by extending `LabelAbstract`.
