...
In tutti i modelli è incluso un service worker per memorizzare nella cache le risorse esterne (immagini, suoni, video, ecc.) utilizzate nel gioco. Ciò migliora le prestazioni e riduce i tempi di caricamento per i giocatori.

</Callout>

Per le risorse ospitate online, il service worker le memorizza nella cache locale sul dispositivo dell'utente dopo il primo download. Ciò significa che le richieste successive per la stessa risorsa verranno gestite dalla cache, con tempi di caricamento più rapidi e un utilizzo ridotto dei dati.
