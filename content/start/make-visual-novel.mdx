...
<Accordion title="What does `mc` mean?" id="what-does-mc-mean">
...
For more information on how to create and use characters you can consult: <DynamicLink href="/start/character">Characters</DynamicLink>
...
It is also possible to create a "pause" effect, hiding the dialogue card for a moment. In _ink_, you can use <DynamicLink href="/ink/other-narrative-features#pause">`# pause`</DynamicLink>, while in JS/TS you can <DynamicLink href="/start/dialogue#delete">set `narration.dialogue` to `undefined`</DynamicLink>.
...
In _ink_, `_input_value_` is a special variable that holds the value entered by the player. You can read more about it <DynamicLink href="/ink/input">here</DynamicLink>.
...
In _ink_, you can use square brackets (e.g. `[sly]`) to take advantage of the <DynamicLink href="/ink/replacement">text replacement</DynamicLink> mechanism: by default, the templates check whether the text between the brackets matches a character `id`, and if so, replace it with that character's name.
...
<Accordion title="Smart asset loading" id="smart-asset-loading">

In our case we saved the game images on a hosting service. For this reason the asset loading is not timely.
...
<Accordion title="Use transitions" id="use-transitions">
...
In this example, we use the move in/out transition to make `james` enter the scene from outside the canvas. In _ink_, this is done by appending `with movein` (and its parameters) to the `# show` command, while in JS/TS you can use the `transitions.moveIn`/`transitions.moveOut` functions.
...
To make the visual novel more dynamic, you can use animations. I recommend using TypeScript if you need to set many properties, as this gives you more control, more functionality, and type feedback.
...
To take `steph` out/in, I will use the `transitions.moveOut` and `transitions.moveIn` functions. For the mirror effect, I will use the `canvas.animate` function.
...
Just for explanation purposes, this is what the equivalent would look like in _ink_:
...
I use the `completeOnContinue: true` option to ensure the animation completes before the next `step` is executed. For `transitions.moveIn`, being a transition, `completeOnContinue` is true by default.

:::tip

For more information about how to use animations, see <DynamicLink href="/start/canvas-animations-effects">here</DynamicLink>.
