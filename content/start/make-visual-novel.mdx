...
In this example, we use the move in/out transition to make `james` enter the scene from outside the canvas. In _ink_, this is done by appending `with movein` (and its parameters) to the `# show` command, while in JS/TS you can use the `moveIn`/`moveOut` functions.
