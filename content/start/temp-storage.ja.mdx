...
To set a temporary variable, use `storage.setTempVariable`. この関数は以下のパラメータを持ちます：
...
To remove a temporary variable, use `storage.removeTempVariable`. この関数は以下のパラメータを持ちます：
