...
In diesem Beispiel verwenden wir den move in/out-Übergang, damit `james` von außerhalb der Canvas in die Szene eintritt. In *ink*, this is done by appending `with movein` (and its parameters) to the `# show` command, while in JS/TS you can use the `transitions.moveIn`/`transitions.moveOut` functions.
...
To take `steph` out/in, I will use the `transitions.moveOut` and `transitions.moveIn` functions. Für den Spiegeleffekt werde ich die Funktion `canvas.animate` verwenden.
...
Ich verwende die Option `completeOnContinue: true`, um sicherzustellen, dass die Animation abgeschlossen wird, bevor der nächste `step` ausgeführt wird. For `transitions.moveIn`, being a transition, `completeOnContinue` is true by default.
