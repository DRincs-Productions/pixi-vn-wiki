...
***ink*** is a simple scripting language for interactive stories, used in games like 80 Days, Heaven's Vault, and Sorcery! Learn more on the [*ink* website](https://www.inklestudios.com/ink/).
...
After installing, use [`importInkText`](/jsdoc/pixi-vn-ink/index/functions/importInkText) to load your ***ink* script**:
...
The ***ink* + Pixi’VN integration** ships a set of [Agent Skills](https://www.skills.sh/) that teach AI coding assistants (like Claude Code) the full ***ink*** language plus every Pixi’VN-specific detail on this page (hashtag commands, `importInkText`, storage mapping, and every deliberate difference from native *ink*). Unlike the main <DynamicLink href="/start#agent-skills">Pixi’VN skill set</DynamicLink>, where each skill can be installed independently, these skills are meant to travel together — install all of them at once with:
...
`INCLUDE` is an *ink* statement to include another *ink* file. In Pixi’VN, you should use [`importInkText`](/jsdoc/pixi-vn-ink/index/functions/importInkText) to load multiple *ink* files instead, so `INCLUDE` is ignored.
