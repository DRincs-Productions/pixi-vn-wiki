...
description: |
  Explains Pixi’VN usage of native ink features such as `pause` and `continue` and how they affect `steps` and dialogue flow.
---
...
To present the dialogue and the choices in a single `step`, append `<># continue` to the dialogue line. This keeps the UI compact and avoids an extra `step`.
