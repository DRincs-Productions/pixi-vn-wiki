...
The **_ink_ + Pixi’VN integration** supports native _ink_ choices without introducing new choice types. For full details on choice syntax and behavior, refer to the [official ink documentation](https://www.inklestudios.com/ink/).

:::

You can ask the player to make a choice in _ink_ using a list of `*` or `+` bullets, followed by the choice text. The choice text is displayed to the player and they select one option to continue the story.
...
<Accordion title="Fallback choices" id="fallback-choices">
...
where:
    knot_name = the name of the knot to jump to when no other choice is available
```
...
<Accordion title="Conditional choices" id="conditional-choices">
