...
These named constants are globally accessible throughout the ***ink*** script, just like enum members in TypeScript/JavaScript.
...
For example, `VAR inventory = (sword, key)` creates an `inventory` list that initially contains `sword` and `key`.
...
The lecturer has {LIST_VALUE(deafening) - LIST_VALUE(lecturersVolume)} notches left.
...
{ GameEvents ? openedCasket: You remember opening the casket. }
{ GameEvents ? (foundSword, metGorgon): You are well prepared. }
```
...
{ PancakeState == batter_mix: The batter is ready. }
{ PancakeState < ready_to_eat: Keep cooking! }
~ PancakeState++
```
...
{ Facts ? Fogg_is_fairly_odd: I smiled politely. | I frowned. }
