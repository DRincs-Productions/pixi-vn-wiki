...
The **_ink_ + Pixi’VN integration** introduces a `# script` that allows you to request an <DynamicLink href="/start/input">input prompt</DynamicLink> from the user. Per fare ciò, è necessario utilizzare la seguente sintassi:
...
The value will be saved in storage with the following system key `_input_value_`. So you can access the value with `_input_value_`.
