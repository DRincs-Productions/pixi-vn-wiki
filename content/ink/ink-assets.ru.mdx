...
Как более подробно описано <DynamicLink href="/start/assets-management">здесь</DynamicLink>, рекомендуется инициализировать матрицу активов в начале проекта, загрузить активы и продумать, когда они должны быть загружены.

В ink, пользователь имеет меньше контроля чем, например, в Javascript/Typescript над тем когда и как загружать активы. В **_ink_**, pixi-vn предоставляет единственную функцию, которая <DynamicLink href="/start/assets-management#load-assets-before-a-label-starts">загружает активы перед запуском меток</DynamicLink>.
...
- `[assets or bundle]`: It is the type of asset that you want to load. The available types are:
  - `assets`: Загружает активы.
  - `bundle`: Загружает пакет.
