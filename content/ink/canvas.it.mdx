...
- `[list of URL/path of the image]`: It is the list of URL/path of the images that you want to show. Se hai inizializzato la <DynamicLink href="/start/assets-management#initialize-the-asset-matrix-at-project-start">matrice delle risorse</DynamicLink>, puoi utilizzare l'alias della texture. Tieni presente che per scrivere `https://` in **_ink_** devi usare `https:\/\/` perché `//` è considerato un commento in **_ink_**.
