...
- `operation`: l'operazione da eseguire. Attualmente supportati:
  ...
  - `pause`: Mettere in pausa un componente canvas. (Per saperne di più [qui](#pause))
  - `resume`: Riprendi un componente canvas. (Per saperne di più [qui](#resume))
...
- `component type`: il tipo di componente. Tipi disponibili:
  ...
  - <DynamicLink href="/start/canvas-text">`text`</DynamicLink>
- `alias`: L'<DynamicLink href="/start/canvas-alias">alias</DynamicLink> per identificare il componente. Utilizza le doppie virgolette se l'alias contiene spazi.
- `source` (Opzionale): il significato varia in base al componente:
  - `image` e `video`: URL o alias dell'assets per la texture.
  - `text`: il testo da visualizzare.
  - `imagecontainer`: un elenco di alias/URL delle texture (ad esempio `[url1 url2 url3]`).
...
### Mettere in pausa e riprendere un video
