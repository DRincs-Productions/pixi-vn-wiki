...
description: |
  This page explains the native ink scripting language, its syntax, features, and how to write interactive stories using ink.
---

<Callout type="info">

The entire page was copied and pasted from the [official ink documentation](https://github.com/inkle/ink/blob/master/Documentation/WritingWithInk.md).
...
You can also include content at the top of a knot outside of any stitch. However, you need to remember to divert out of it - the engine _won't_ automatically enter the first stitch once it's worked its way through the header content.
...
Note that the test `knot_name` is true if _any_ stitch inside that knot has been seen.
...
You can use several logical tests on an option; if you do, _all_ the tests must all be passed for the option to appear.
...
is actually testing an _integer_ and not a true/false flag. A knot or stitch used this way is actually an integer variable containing the number of times the content at the address has been seen by the player.
...
... and have the game react to content the player saw _just now_.
...
In truth, all content in ink is a weave, even if there are no gathers in sight. That means you can label _any_ option in the game with a bracket label, and then reference it using the addressing syntax. In particular, this means you can test _which_ option a player took to reach a particular outcome.
...
Options can also be diverted to: but the divert goes to the output of having chosen that choice, _as though the choice had been chosen_. So the content printed will ignore square bracketed text, and if the option is once-only, it will be marked as used up.
...
Ink can generate random integers if required using the RANDOM function. RANDOM is authored to be like a dice (yes, pendants, we said _a dice_), so the min and max values are both inclusive.
...
Threads allow you to compose sections of content from multiple sources in one go. Ad esempio:
...
Note that global variables are _not_ forked, including the read counts of knots and stitches.
...
When a list is defined, the values are listed in an order, and that order is considered to be significant. In fact, we can treat these values as if they _were_ numbers. (That is to say, they are enums.)
...
does not mean neither Adams nor Bernard is present, only that they are not _both_ present (and arguing).
...
Lists don't _have_ to contain multiple values. If you want to use a list as a state-machine, the examples above will all work - set values using `=`, `++` and `--`; test them using `==`, `<`, `<=`, `>` and `>=`. These will all work as expected.

### The "full" list

Note that `LIST_COUNT`, `LIST_MIN` and `LIST_MAX` are refering to who's in/out of the list, not the full set of _possible_ doctors. We can access that using
...
`LIST_A >= LIST_B` is _not_ the same as `LIST_A > LIST_B or LIST_A == LIST_B`.
...
To test for "some overlap" between lists, we use the overlap operator, `^`, to get the _intersection_.
...
produces:
 ...
 On one table, a headline blares out WHO IS THE BATMAN? AND _WHO_ IS HIS BARELY-REMEMBERED ASSISTANT?
