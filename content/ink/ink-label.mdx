...
In the Pixi’VN library, a knot corresponds to a [label](/start/labels#label). The ***ink* + Pixi’VN integration**, in addition to giving the possibility to run ***ink* nodes**, also allows you to call knots (or labels) written in another [narrative language or Javascript/TypeScript](/start/narration).
...
The **native *ink* language** does not have a possibility to ["call" a knot](/start/labels#call-a-label) without close the current knot and open the new one. This is why the ***ink* + Pixi’VN integration** introduces the `# call` script. This script is ignored by **Inky**.
