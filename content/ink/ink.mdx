...
The novice developers can use a [*ink* template](/start/getting-started#project-initialization) to start developing just with ***ink***, and then gradually learn Javascript/Typescript to create more complex features.

## Start using *ink* in Pixi’VN

If you have not created a project yet then it is recommended to use a [Pixi’VN + *ink* template](/start/getting-started#project-initialization) to start developing with ***ink***.
...
Now you can run the ***ink* knot** (or label) with [Pixi’VN functions](/start/labels#run-a-label).
