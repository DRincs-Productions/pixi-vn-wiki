...
Are there differences between `labels` and `knots`? No, they are the same thing. In Pixi’VN, when you create a `knot` with ***ink***, a label will be created internally and saved in the registry.
