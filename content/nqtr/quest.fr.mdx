...
In all templates, the `Quest` class is already defined in the file `models/nqtr/Quest.ts`. Vous pouvez l'utiliser directement ou la modifier selon vos besoins.
...
Now you can create a class `Quest` that extends `QuestStoredClass` and implements the `QuestInterface`. (Pour plus d'informations sur la création d'une classe en TypeScript, lisez [la documentation officielle](https://www.typescriptlang.org/docs/handbook/2/classes.html))
