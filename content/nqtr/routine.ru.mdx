...
In all templates, the `Commitment` class is already defined in the file `models/nqtr/Commitment.ts`. Вы можете использовать его напрямую или изменить под свои нужды.
...
Now you can create a class `Commitment` that extends `CommitmentStoredClass` and implements the `CommitmentInterface`. (Дополнительную информацию о создании классов в TypeScript можно найти в [официальной документации](https://www.typescriptlang.org/docs/handbook/2/classes.html))
