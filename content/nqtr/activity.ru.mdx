...
In all templates, the `Activity` class is already defined in the file `models/nqtr/Activity.ts`. Вы можете использовать его напрямую или изменить под свои нужды.
...
Now you can create a class `Activity` that extends `ActivityStoredClass` and implements the `ActivityInterface`. (Дополнительную информацию о создании классов в TypeScript можно найти в [официальной документации](https://www.typescriptlang.org/docs/handbook/2/classes.html))
