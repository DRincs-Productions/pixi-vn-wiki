...
In all templates, the `Activity` class is already defined in the file `models/nqtr/Activity.ts`. そのまま使用するか、ニーズに合わせて変更できます。
...
Now you can create a class `Activity` that extends `ActivityStoredClass` and implements the `ActivityInterface`. （TypeScript でのクラス作成方法の詳細については、[公式ドキュメント](https://www.typescriptlang.org/docs/handbook/2/classes.html)をお読みください）
