...
In all templates, the `Activity` class is already defined in the file `models/nqtr/Activity.ts`. 直接使用するか、それを修正して自分のニーズに合わせることができます。
...
Now you can create a class `Activity` that extends `ActivityStoredClass` and implements the `ActivityInterface`. （TypeScriptでクラスを作成する方法の詳細については、[公式ドキュメント](https://www.typescriptlang.org/docs/handbook/2/classes.html)を読んでください）
