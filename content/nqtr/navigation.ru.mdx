...
<Callout title="Терминология" type="info">
...
In all templates, the `Room` / `Location` / `Map` class is already defined in the file `models/nqtr/Room.ts` / `models/nqtr/Location.ts` / `models/nqtr/Map.ts`. Вы можете использовать его напрямую или изменить под свои нужды.
