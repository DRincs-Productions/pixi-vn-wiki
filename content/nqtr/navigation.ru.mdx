...
In all templates, the `Room` / `Location` / `Map` class is already defined in the file `models/nqtr/Room.ts` / `models/nqtr/Location.ts` / `models/nqtr/Map.ts`. Вы можете использовать это напрямую или изменить его в соответствии с вашими потребностями.
