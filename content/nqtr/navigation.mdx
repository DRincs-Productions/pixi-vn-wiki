...
* `id`: A unique identifier (string). It is used to reference the room in the game (must be unique).
...
* `props`: An object with the room's properties:
  ...
  * `activities` (Optional): The activities that are available in this room.
...
* `id`: A unique identifier (string). It is used to reference the location in the game (must be unique).
...
* `props`: An object with the location's properties:
  ...
  * `activities` (Optional): The activities that are available in this location.
...
* `id`: A unique identifier (string). It is used to reference the map in the game (must be unique).
* `props`: An object with the map's properties:
  * `name` (Optional): The map's name.
  * `image` (Optional): The map's image URL.
  * `activities` (Optional): The activities that are available in this map.
