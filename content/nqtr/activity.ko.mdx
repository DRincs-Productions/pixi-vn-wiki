...
In all templates, the `Activity` class is already defined in the file `models/nqtr/Activity.ts`. 직접 사용하거나 필요에 맞게 수정할 수 있습니다.
...
Now you can create a class `Activity` that extends `ActivityStoredClass` and implements the `ActivityInterface`. (TypeScript에서 클래스를 만드는 방법에 대한 자세한 내용은 [공식 문서](https://www.typescriptlang.org/docs/handbook/2/classes.html)를 참조하세요)
