...
In all templates, the `Commitment` class is already defined in the file `models/nqtr/Commitment.ts`. 直接使用するか、それを修正して自分のニーズに合わせることができます。
...
Now you can create a class `Commitment` that extends `CommitmentStoredClass` and implements the `CommitmentInterface`. （TypeScriptでクラスを作成する方法の詳細については、[公式ドキュメント](https://www.typescriptlang.org/docs/handbook/2/classes.html)を読んでください）
