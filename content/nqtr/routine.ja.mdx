...
In all templates, the `Commitment` class is already defined in the file `models/nqtr/Commitment.ts`. そのまま使用するか、ニーズに合わせて変更できます。
...
Now you can create a class `Commitment` that extends `CommitmentStoredClass` and implements the `CommitmentInterface`. （TypeScript でのクラス作成方法の詳細については、[公式ドキュメント](https://www.typescriptlang.org/docs/handbook/2/classes.html)をお読みください）
