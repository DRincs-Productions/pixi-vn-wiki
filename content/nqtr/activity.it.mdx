...
Con `activity` si intendono le azioni che il giocatore può intraprendere, collegabili a un <DynamicLink href="/nqtr/navigation">elemento di navigazione</DynamicLink>. Lo sviluppatore deciderà come il giocatore vi possa interagire.

## Inizializzazione

Per inizializzare una `activity`, crea una nuova istanza della classe `ActivityBaseModel` (o la tua [classe personalizzata](#custom-class)) e aggiungila al dizionario delle attività del gioco all'inizializzazione.
...
In all templates, the `Activity` class is already defined in the file `models/nqtr/Activity.ts`. Puoi usarla direttamente o modificarla in base alle tue esigenze.
...
Now you can create a class `Activity` that extends `ActivityStoredClass` and implements the `ActivityInterface`. (Per ulteriori informazioni su come creare una classe in TypeScript, leggi [la documentazione ufficiale](https://www.typescriptlang.org/docs/handbook/2/classes.html))
