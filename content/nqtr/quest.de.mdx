...
In all templates, the `Quest` class is already defined in the file `models/nqtr/Quest.ts`. Sie können sie direkt verwenden oder nach Bedarf anpassen.
...
Now you can create a class `Quest` that extends `QuestStoredClass` and implements the `QuestInterface`. (Weitere Informationen zur Erstellung einer Klasse in TypeScript finden Sie in der [offiziellen Dokumentation](https://www.typescriptlang.org/docs/handbook/2/classes.html))
