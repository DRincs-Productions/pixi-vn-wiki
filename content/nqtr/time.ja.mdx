...
To initialize the time system, you need use the `initialize` method of the `timeTracker` instance. この関数は以下のパラメータを持ちます：
...
To increment the current `date` by a specific amount, use the `increaseDate` method. この関数は以下のパラメータを持ちます：
