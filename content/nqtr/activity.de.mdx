...
In all templates, the `Activity` class is already defined in the file `models/nqtr/Activity.ts`. Sie können sie direkt verwenden oder nach Bedarf anpassen.
...
Now you can create a class `Activity` that extends `ActivityStoredClass` and implements the `ActivityInterface`. (Weitere Informationen zur Erstellung einer Klasse in TypeScript finden Sie in der [offiziellen Dokumentation](https://www.typescriptlang.org/docs/handbook/2/classes.html))
