...
In all templates, the `Commitment` class is already defined in the file `models/nqtr/Commitment.ts`. Vous pouvez l'utiliser directement ou la modifier selon vos besoins.
...
Now you can create a class `Commitment` that extends `CommitmentStoredClass` and implements the `CommitmentInterface`. (Pour plus d'informations sur la création d'une classe en TypeScript, lisez [la documentation officielle](https://www.typescriptlang.org/docs/handbook/2/classes.html))
