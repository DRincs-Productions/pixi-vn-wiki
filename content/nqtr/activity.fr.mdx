...
In all templates, the `Activity` class is already defined in the file `models/nqtr/Activity.ts`. Vous pouvez l'utiliser directement ou la modifier selon vos besoins.
...
Now you can create a class `Activity` that extends `ActivityStoredClass` and implements the `ActivityInterface`. (Pour plus d'informations sur la création d'une classe en TypeScript, lisez [la documentation officielle](https://www.typescriptlang.org/docs/handbook/2/classes.html))
