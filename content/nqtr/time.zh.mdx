...
To initialize the time system, you need use the `initialize` method of the `timeTracker` instance. 该函数具有以下参数：
...
To increment the current `date` by a specific amount, use the `increaseDate` method. 该函数具有以下参数：
