...
In all templates, the `Quest` class is already defined in the file `models/nqtr/Quest.ts`. Вы можете использовать его напрямую или изменить под свои нужды.
...
Now you can create a class `Quest` that extends `QuestStoredClass` and implements the `QuestInterface`. (Дополнительную информацию о создании классов в TypeScript можно найти в [официальной документации](https://www.typescriptlang.org/docs/handbook/2/classes.html))
