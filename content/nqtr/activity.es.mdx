...
In all templates, the `Activity` class is already defined in the file `models/nqtr/Activity.ts`. Puedes usarla directamente o modificarla según tus necesidades.
...
Now you can create a class `Activity` that extends `ActivityStoredClass` and implements the `ActivityInterface`. (Para más información sobre cómo crear una clase en TypeScript, lee [la documentación oficial](https://www.typescriptlang.org/docs/handbook/2/classes.html))
