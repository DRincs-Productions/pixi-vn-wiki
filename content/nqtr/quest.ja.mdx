...
To start a `quest`, you can use the `start` method of the Quest class. This method will set the current stage to the first stage of the quest and trigger the `onStart` action defined in the quest. この関数は以下のパラメータを持ちます：
...
To go to the next stage of a `quest`, you can use the `continue` method of the Quest class. This method will set the current stage to the next stage of the quest and trigger the `onContinue` action defined in the quest. この関数は以下のパラメータを持ちます：
...
In all templates, the `Quest` class is already defined in the file `models/nqtr/Quest.ts`. 直接使用するか、それを修正して自分のニーズに合わせることができます。
...
Now you can create a class `Quest` that extends `QuestStoredClass` and implements the `QuestInterface`. （TypeScriptでクラスを作成する方法の詳細については、[公式ドキュメント](https://www.typescriptlang.org/docs/handbook/2/classes.html)を読んでください）
