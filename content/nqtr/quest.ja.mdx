...
In all templates, the `Quest` class is already defined in the file `models/nqtr/Quest.ts`. そのまま使用するか、ニーズに合わせて変更できます。
...
Now you can create a class `Quest` that extends `QuestStoredClass` and implements the `QuestInterface`. （TypeScript でのクラス作成方法の詳細については、[公式ドキュメント](https://www.typescriptlang.org/docs/handbook/2/classes.html)をお読みください）
