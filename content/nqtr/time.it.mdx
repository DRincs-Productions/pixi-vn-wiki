...
To initialize the time system, you need use the `initialize` method of the `timeTracker` instance. Questa funzione ha i seguenti parametri:
...
To increment the current `date` by a specific amount, use the `increaseDate` method. Questa funzione ha i seguenti parametri:
...
<Accordion title="Image that changes based on the time period" id="image-time-period">
...
```ts tab="models/ImageTimeSlots.ts"
import { timeTracker } from "@drincs/nqtr";
