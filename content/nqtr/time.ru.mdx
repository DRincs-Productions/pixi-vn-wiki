...
To initialize the time system, you need use the `initialize` method of the `timeTracker` instance. Эта функция принимает следующие параметры:
...
To increment the current `date` by a specific amount, use the `increaseDate` method. Эта функция принимает следующие параметры:
