...
In all templates, the `Room` / `Location` / `Map` class is already defined in the file `models/nqtr/Room.ts` / `models/nqtr/Location.ts` / `models/nqtr/Map.ts`. 직접 사용하거나 필요에 맞게 수정할 수 있습니다.
...
Now you can create a class `Room` / `Location` / `Map` that extends `RoomStoredClass` / `LocationStoredClass` / `MapStoredClass` and implements the `RoomInterface` / `LocationInterface` / `MapInterface`. (TypeScript에서 클래스를 만드는 방법에 대한 자세한 내용은 [공식 문서](https://www.typescriptlang.org/docs/handbook/2/classes.html)를 참조하세요)
...
<Accordion title="How do I set a room for a location that will open by default?" id="how-set-default-room-for-location">
