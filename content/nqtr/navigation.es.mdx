...
In all templates, the `Room` / `Location` / `Map` class is already defined in the file `models/nqtr/Room.ts` / `models/nqtr/Location.ts` / `models/nqtr/Map.ts`. Puedes usarla directamente o modificarla según tus necesidades.
...
Now you can create a class `Room` / `Location` / `Map` that extends `RoomStoredClass` / `LocationStoredClass` / `MapStoredClass` and implements the `RoomInterface` / `LocationInterface` / `MapInterface`. (Para más información sobre cómo crear una clase en TypeScript, lee [la documentación oficial](https://www.typescriptlang.org/docs/handbook/2/classes.html))
