...
In all templates, the `Quest` class is already defined in the file `models/nqtr/Quest.ts`. Puedes usarla directamente o modificarla según tus necesidades.
...
Now you can create a class `Quest` that extends `QuestStoredClass` and implements the `QuestInterface`. (Para más información sobre cómo crear una clase en TypeScript, lee [la documentación oficial](https://www.typescriptlang.org/docs/handbook/2/classes.html))
