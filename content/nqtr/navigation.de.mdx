...
<Callout title="Nomenklatur" type="info">
...
In all templates, the `Room` / `Location` / `Map` class is already defined in the file `models/nqtr/Room.ts` / `models/nqtr/Location.ts` / `models/nqtr/Map.ts`. Sie können sie direkt verwenden oder nach Bedarf anpassen.
