...
Per creare una proprietà che memorizzi il suo valore nell'archivio di gioco, puoi creare [Getter/Setter](https://www.typescriptlang.org/docs/handbook/2/classes.html#getters--setters) e utilizzare i metodi `this.getStorageProperty()`/`this.setStorageProperty()`. (See the file `Room.ts`/`Location.ts`/`Map.ts`)
