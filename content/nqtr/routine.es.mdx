...
In all templates, the `Commitment` class is already defined in the file `models/nqtr/Commitment.ts`. Puedes usarla directamente o modificarla según tus necesidades.
...
Now you can create a class `Commitment` that extends `CommitmentStoredClass` and implements the `CommitmentInterface`. (Para más información sobre cómo crear una clase en TypeScript, lee [la documentación oficial](https://www.typescriptlang.org/docs/handbook/2/classes.html))
