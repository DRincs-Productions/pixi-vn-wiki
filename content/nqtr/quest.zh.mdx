...
In all templates, the `Quest` class is already defined in the file `models/nqtr/Quest.ts`. 您可以直接使用它或根据需要修改它。
...
Now you can create a class `Quest` that extends `QuestStoredClass` and implements the `QuestInterface`. （有关如何在 TypeScript 中创建类的更多信息，请阅读[官方文档](https://www.typescriptlang.org/docs/handbook/2/classes.html)）
