...
To do this, you just need to create a [custom character](/start/character#custom-character) or modify the existing one (it is already present in the templates).
...
For example you can add a script with the following syntax and convert it to a [show imagecontainer script](/ink/ink-canvas#show-a-image-container-in-ink):
