...
In Pixi'VN, in alcune funzionalità avanzate, è necessario utilizzare i decorator.

Per impostazione predefinita, TypeScript non abilita l'uso dei decorator. Per abilitare l'uso dei decorator in TypeScript, è necessario aggiungere la seguente configurazione al file `tsconfig.json`:
...
Collegare un personaggio a un'immagine da aggiungere al canvas è una caratteristica comune nelle visual novel. Può essere utile, ad esempio, per mostrare l'espressione del personaggio.

Per fare ciò, è sufficiente creare un <DynamicLink href="/start/character#custom-class">personaggio personalizzato</DynamicLink> o modificare quello esistente (è già presente nei modelli).
Consiglio di aggiungere un array di stringhe contenenti i link/alias delle immagini che compongono il personaggio (corpo, testa...) e di utilizzare un <DynamicLink href="/start/canvas-image-container">ImageContainer</DynamicLink> quando è necessario visualizzare il personaggio.
...
import { HashtagScript } from '@drincs/pixi-vn-ink'
import { getCharacterById } from '@drincs/pixi-vn'

HashtagScript.add((script, convertListStringToObj) => {
    // ...
...
Pixi’VN è rilasciato sotto la **GNU Lesser General Public License v2.1 (LGPL 2.1)**.\
