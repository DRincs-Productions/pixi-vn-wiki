...
description: Domande frequenti riguardo Pixi'VN
---

Alcune domande comuni che potresti porti.

<Accordions>

<Accordion title="Come abilitare i decoratori in TypeScript?" id="enable-typescript-decorators">

Su Pixi'VN, in alcune funzionalità avanzate, è necessario utilizzare i decoratori.

Di default, TypeScript non ne consente l'utilizzo. Per abilitare l'utilizzo dei decoratori su TypeScript, devi aggiungere la seguente configurazione al file `config.json`.
...
<Accordion title="Salta gli `steps` e vai avanti automaticamente" id="skip-auto">
...
L'implementazione suggerita per questa funzionalità è creare la seguente funzione:
...
<Accordion title="Come collegare un personaggio a un'immagine da aggiungere al canvas?" id="link-character-to-image">

Collegare un personaggio a un'immagine da aggiungere all'area di lavoro è una caratteristica comune nelle visual novel. Può ad esempio tornare utile per mostrare l'espressione del personaggio.
...
<Accordion title="Keyboard shortcuts (tasti di scelta rapida)" id="hotkeys">

I suggest the following hotkeys, based on the ones used in the [pixi-vn-react-template](https://github.com/DRincs-Productions/pixi-vn-react-template):

* `Space` or `Enter` (hold): Skip through the dialogue while pressed.
* `Space` or `Enter` (release): Continue the dialogue.
* `F5` or `Ctrl + S`: Quick save the game.
* `F9` or `Ctrl + L`: Quick load the most recent save.
* `Esc`: Open the settings modal.
* `H`: Open the history modal.
* `K`: Open the controls/hotkeys list.
* `Tab`: Toggle the quick actions wheel.
* `Arrow Up` / `Arrow Down`: Navigate the choices in a choice menu.

Function keys (`F5`, `F9`) are used as the primary bindings because they are rarely intercepted by the browser, with `Ctrl` + `S`/`Ctrl` + `L` provided as alternatives for keyboards or setups without function keys.

</Accordion>

<Accordion title="Collegamento di eventi ai tasti per consentire i tasti di scelta rapida" id="keymaps-events">
