...
`narration.choices` (getter) was `StoredIndexedChoiceInterface[] | undefined`; it now returns a namespace object, and the array moved to `.list`. The _setter_, `narration.choices = [...]`, is unchanged — it's a shortcut for `.list`:
