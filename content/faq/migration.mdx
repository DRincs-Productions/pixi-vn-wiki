...
In questa versione è stata rivisto da zero l'impianto musicale perchè era molto limitante. In questa nuova versione è stata aggiunta il componente `channel` e si è suddiviso il sistema in quattro elementi che adenpiono ad un lavoro specifico, e non come prima che potevano essere usati in più modi creando instabilita nel sistema (`sound` (manager), `channels`, `media`, sound `assets`).
...
è stato migliorato ancke il sistema di keyword dello storage, sfruttando la l'uso comune per la nomenclatura delle key usando `:` come separatore, e aggiungendo la possibilità di usare un prefisso per identificare meglio le chiavi (ad esempio `character:alice:friendship`).

Infine è stana migliorata la gestione degli errori, aggiungendo la possibilità di gestire gli errori aggiungendo degli handler specifici.
...
{
    // ...
    "___nqtr-temporary_commitment___": [ // [!code --]
        "alice_sleep", // [!code --]
        "alice_eat", // [!code --]
    ], // [!code --]
    "___nqtr-temporary_commitment___": { // [!code ++]
