...
In questa rilascio, è stata modificato il modo per collegare un `commitment` a una `room`, ora direttamente nella `room` potrai definire la sua routine. Quindi invece di passare la `room` nel costruttore del `commitment`, potrai definire la routine direttamente nel costruttore della `room`.

Questa modifica è stata fatta per semplificare la gestione delle routine e per rendere più chiaro il legame tra `commitment` e `room`.
