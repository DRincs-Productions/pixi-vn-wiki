...
- You can't set a listener with the `on` method, but must use the [onEvent](/start/canvas-functions#add-a-listener-for-a-given-event) to add a listener.
