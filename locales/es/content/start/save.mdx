...
To load the game progress, you can use the `Game.restoreGameState` function. This function receives a object with the game data and a function `navigate: (path: string) => void` that will be called with the [URL Path or Route](/start/interface#what-is-the-url-path-and-routes) of the saved step, so you can use it to navigate to the saved [UI screen](/start/interface-navigate).
