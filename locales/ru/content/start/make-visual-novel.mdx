...
Поскольку Pixi'VN даёт вам возможность написать собственное повествование, выбрав один или несколько [доступных языков повествования](/start/narration), на каждом этапе разработки будут создаваться примеры для каждого из доступных на данный момент языков.

## Создать новый проект

Первый шаг - создание нового проекта. Вы можете найти больше информации о том, как создать новый проект на основе шаблона [здесь](/start/getting-started#project-initialization). Мы будем использовать шаблон "Визуальная новелла - React".
...
Теперь мы определим персонажей этой истории. Для этого в файле `/values/characters.ts` мы определим персонажей, которых будем использовать. Больше информации о том, как создавать и использовать персонажей, вы можете найти на странице: [Персонажи](/start/character)

Что означает `гг`? `гг` - распространенное сокращение от "Главный Герой". В визуальных романах принято использовать `mc` (Main Character) в качестве имени главного героя.
...
```ts tab="App.tsx"
// Помните импортировать файл персонажа как минимум один раз в вашем проекте. // [!code focus]
...
Теперь мы можем приступить к написанию "первого черновика" [повествования] (/start/narration) визуальной новеллы.
Мы создадим первую ["часть"](/start/labels) под названием `start`, который будет началом игры.

После этого мы можем написать [диалоги](/start/dialogue.md), которые последуют в нашей визуальной новелле. Выбранный нами шаблон поддерживает [язык разметки markdown](/start/markup-markdown.md) ([Язык Разметки в _ink_](/ink/ink-markup)), поэтому мы будем использовать его для нашего повествования.
...
### Разделите повествование на части

Не рекомендуется создавать очень длинные части (даже для линейных визуальных новелл), желательно создать несколько небольших частей и "вызывать" их по мере необходимости с помощью [функций управления игровым потоком](/start/labels-flow.md) ([_ink_ knot (или метка)](/ink/ink-label)).

По этой причине, даже если в нашем случае история линейна, она будет разделена на две части, одна из которых будет та, которую мы только что создали, а вторая будет называться `second_part`.
...
Для этого мы воспользуемся [меню выбора](/start/choices).
...
Для этого я попрошу игрока [заполнить поле ввода, используя возможности Pixi'VN](/start/input.md) ([Используйте подсказку ввода в _ink_](/ink/ink-input)).

После получения входного значения вы можете [установить имя персонажа](/start/character.md#edit-characters-in-the-game), используя полученное значение ([Редактировать имя персонажа в _ink_](/ink/ink-character#edit-character-name-in-dialogues)).
...
Теперь мы можем использовать имена персонажей в диалогах ([Использовать имя персонажа в диалогах в _ink_](/ink/ink-character#use-character-name-in-dialogues)).
...
В визуальных новеллах часто бывает полезно вставить текст в текущий диалог. Например, чтобы приостановить разговор и продолжить его на следующем этапе. Для этого мы можем воспользоваться [функцией клея](/start/dialogue#dialogue-glue).
...
Один из первых шагов - выбор, где будут сохранены текстуры визуальной новеллы. В данном случае мы сохраним текстуры в хранилище Firebase (хостинговый сервис).
...
Перед использованием текстур настоятельно рекомендуется [инициализировать матрицу текстур](/start/assets-management#initialize-the-asset-matrix-at-project-start).

По умолчанию, как вы можете видеть в файле `assets/manifest.ts`, все шаблоны в `onLoadingLabel` пытаются загрузить в фоновом режиме "bundle assets" с псевдонимом, равным id метки. Поэтому рекомендуется добавить в `manifest` "bundle assets" для каждого ярлыка с псевдонимом, равным id метки, и содержащим изображения, используемые в этой метке.
...
export async function defineAssets() {
    ...
    // Игра не запустится, пока эти текстуры не будут загружены.
    await Assets.loadBundle("main_menu");

    // Игра сразу начнётся, но эти текстуры будут загружены в фоне.
...
В нашем случае спрайты персонажей состоят из 3 изображений: тела, глаз и рта. Затем мы используем [ImageContainer](/start/canvas-image-container) для создания персонажа.

Вы можете найти больше информации о том, как добавлять компоненты холста в [этой документации](/start/canvas-components.md) ([Использование компонентов холста в _ink_](/ink/ink-canvas)).
...
Чтобы игрок не получал слишком много загрузок, мы должны сгруппировать их в определенные фазы игры. В моем случае я буду загружать наиболее используемые изображения в начале части.

Вы можете найти больше информации о том, как управлять загрузками [здесь](/start/assets-management).
...
Чтобы сделать визуальную новеллу более динамичной, можно использовать переходы для показа изображений. Вы можете найти больше информации об использовании переходов [здесь](/start/canvas-transition) ([Использование переходов в _ink_](/ink/ink-canvas#show-a-canvas-component-with-transition-in-ink)).
...
Чтобы сделать визуальную новеллу более динамичной, можно использовать анимацию. Вы можете найти больше информации об использовании анимаций [здесь](/start/canvas-animations-effects) ([Использование анимаций в _ink_](/ink/ink-canvas#use-the-effects-in-ink)).

Я рекомендую использовать Typescript, если вам нужно задать много свойств, так у вас будет больше контроля над анимацией, больше функциональности и обратной связи.

В моем случае анимация будет выводить steph из сцены и снова вставлять её в следующем шаге. Я также отзеркалю её по оси x, чтобы убедиться, что она направлена в нужную сторону.

Для вывода/ввода steph я буду использовать функции `moveOut` и `moveIn`. Для зеркального эффекта я буду использовать тикер `ZoomTicker`.

Важной особенностью переходов является то, что они на мгновение приостанавливают все анимации, связанные с этим компонентом, и возобновляют их по завершении перехода.

Поэтому в моем случае я использую перед функцией `moveIn` функцию `addTicker`, чтобы добавить тикер `ZoomTicker`. Таким образом, после завершения перехода Steph будет зеркально отображаться на оси x.

Также, поскольку я буду использовать typescript для этой анимации, я создал метку для этой анимации. Чтобы его можно было вызывать и из других языков, не являющихся JS/TS.
...
Теперь я могу вызвать эту метку `animation_01` из основной части `start`. (Как объясняется [здесь](/ink/ink-label#use-the-call-script) из _ink_ я могу вызывать метки, написанные на ts, и наоборот.)
