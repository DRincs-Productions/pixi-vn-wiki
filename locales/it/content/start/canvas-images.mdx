...
- `imageUrl` (Optional): The URL or path of the image. Se hai inizializzato la [matrice delle risorse](/start/assets-management#initialize-the-asset-matrix-at-project-start), puoi utilizzare l'alias della texture.
...
The simplest and fastest method to show an image on the canvas is to use the `showImage` function. Questa funzione è una combinazione delle funzioni `load` e [`canvas.add`](/start/canvas-functions#add-a-canvas-component).
...
- `imageUrl` (Optional): The URL or path of the image. Se hai inizializzato la [matrice delle risorse](/start/assets-management#initialize-the-asset-matrix-at-project-start), puoi utilizzare l'alias della texture. If you don't provide the URL, then the alias is used as the URL.
- `options` (Optional): It corresponds to the `ImageSpriteOptions` interface.
...
To add an image to the canvas, you can use the `addImage` function. It is important to take into account that this function only adds the component to the canvas but does **not show it and does not load its texture**. This function use [`canvas.add`](/start/canvas-functions#add-a-canvas-component) to add the image to the canvas.
