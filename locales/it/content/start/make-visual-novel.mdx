...
The first step is to create a new project. You can find more information on how to create a new project starting from a template [here](/start/getting-started#project-initialization). We will use the template "Visual Novel - React".
...
After that we can write the [dialogues](/start/dialogue.md) that will follow in our visual novel. The template we have chosen supports the [markup language markdown](/start/markup-markdown.md) ([Markup language in _ink_](/ink/ink-markup)) so we will use it for our narration.
...
It is not advisable to create very long labels (even for linear visual novels), but it is advisable to create multiple small labels and "call" them when needed with the [game flow control features](/start/labels-flow.md) ([_ink_ knot (or label)](/ink/ink-label)).
...
To do this, I will ask the player to [complete an input box using Pixi'VN's features](/start/input.md) ([Use the input prompt in _ink_](/ink/ink-input)).

After getting the input value, you can [set the character name](/start/character.md#edit-characters-in-the-game) using the obtained value ([Edit character name in _ink_](/ink/ink-character#edit-character-name-in-dialogues)).
...
Now we could use character names within dialogues ([Use character name in dialogues in _ink_](/ink/ink-character#use-character-name-in-dialogues)).
...
In visual novels, it is often useful to paste text into the current dialogue. For example, to pause a conversation and have it continue in a subsequent step. To do this, we can use the [glue functionality](/start/dialogue#dialogue-glue).
...
Before using an asset it is highly recommended to [initialize the asset matrix](/start/assets-management#initialize-the-asset-matrix-at-project-start).
...
You can find more information on how to add canvas components in [this documentation](/start/canvas-components.md) ([Use canvas components in _ink_](/ink/ink-canvas)).
...
To make the visual novel more dynamic, you can use transitions to show images. You can find more information about using transitions [here](/start/canvas-transition) ([Using transitions in _ink_](/ink/ink-canvas#show-a-canvas-component-with-transition-in-ink)).
...
To make the visual novel more dynamic, you can use animations. You can find more information about how to use animations [here](/start/canvas-animations-effects) ([Using animations in _ink_](/ink/ink-canvas#use-the-effects-in-ink)).
...
Now I can call this label `animation_01` from the main label `start`. (As explained [here](/ink/ink-label#use-the-call-script) from _ink_ I can call labels written in ts and vice versa.)
