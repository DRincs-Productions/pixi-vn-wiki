...
Il componente `ImageContainer` estende il componente [`Container`](/start/canvas-components#base-components), così puoi utilizzare tutti i metodi e le proprietà di `Container`. It is used to add a component composed of multiple images to the canvas and manipulate it as if it were a single image.
