...
firebase.hosting.redirects = redirects.map(({ source, destination }) => ({ source, destination, type: 301 }));
...
vercel.redirects = redirects.map(({ source, destination }) => ({ source, destination, permanent: true }));
